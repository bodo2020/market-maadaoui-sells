...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (existingAuth?.user) return json({ code: "already_migrated" }, 409);

    const suppliedHash = await sha256(password);
    const storedHash = await sha256(String(user.password ?? ""));
    ...
    const { error: clearError } = await supabase
      .from("users")
      .update({ password: legacyMarker })
      .eq("id", user.id);
    if (clearError) console.error("Auth account created but legacy password could not be cleared", clearError.message);
