-- POS operational branch stays the accounting/sales branch.
-- Inventory and pricing are resolved explicitly from branches.inventory_source_branch_id / pricing_source_branch_id.
...
begin
  ...
  ) then raise exception using errcode='42501',message='BRANCH_ACCESS_DENIED'; end if;

  select s.inventory_branch_id,s.pricing_branch_id into v_inventory_branch,v_pricing_branch
...
create or replace function private.adjust_branch_inventory(p_request_id uuid,p_product_id uuid,p_branch_id uuid,p_delta numeric)
returns numeric language plpgsql security definer set search_path='' as $$
declare previous private.pos_inventory_requests%rowtype; fingerprint text; result numeric; v_inventory_branch uuid;
begin
  ...
  if p_request_id is null or p_product_id is null or p_branch_id is null or p_delta is null
    or p_delta::text in ('NaN','Infinity','-Infinity') or p_delta=0 or round(p_delta,3)<>p_delta then
    raise exception using errcode='22023',message='INVALID_STOCK_CHANGE'; end if;
  select s.inventory_branch_id into v_inventory_branch from private.resolve_branch_sources(p_branch_id) s;
  fingerprint:=md5(jsonb_build_array(p_product_id,p_branch_id,v_inventory_branch,p_delta)::text);
  ...
  update public.inventory set quantity=quantity+p_delta
  where product_id=p_product_id and branch_id=v_inventory_branch and quantity+p_delta>=0 returning quantity into result;
...
declare
  previous public.sales%rowtype; saved public.sales%rowtype; p public.products%rowtype;
  x jsonb; deductions jsonb:='[]'; quantity numeric; total numeric; subtotal numeric; discount numeric;
  cash numeric; card numeric; fingerprint text; method text; v_inventory_branch uuid; v_pricing_branch uuid;
begin
  ...
  select s.inventory_branch_id,s.pricing_branch_id into v_inventory_branch,v_pricing_branch from private.resolve_branch_sources(p_branch_id) s;
  fingerprint:=md5(jsonb_build_array(p_branch_id,v_inventory_branch,v_pricing_branch,p_sale)::text);
  ...
  for x in select jsonb_build_object('id',value->>'id','quantity',sum((value->>'quantity')::numeric))
    from jsonb_array_elements(deductions) group by value->>'id' order by value->>'id' loop
    update public.inventory set quantity=inventory.quantity-(x->>'quantity')::numeric
    where product_id=(x->>'id')::uuid and branch_id=v_inventory_branch and inventory.quantity>=(x->>'quantity')::numeric;
    if not found then raise exception using errcode='22023',message='INSUFFICIENT_STOCK'; end if;
  end loop;
  ...
  values(p_request_id,p_sale->'items',auth.uid(),(select name from public.users where id=auth.uid()),p_branch_id,subtotal,discount,total,
    coalesce((p_sale->>'profit')::numeric,0),method,cash,card,p_sale->>'customer_name',p_sale->>'customer_phone',p_sale->>'invoice_number',fingerprint)
  returning * into saved;
  if cash>0 then perform public.add_cash_transaction_api(cash,'deposit','store','مبيعات - فاتورة '||coalesce(saved.invoice_number,saved.id::text),auth.uid(),p_branch_id); end if;
