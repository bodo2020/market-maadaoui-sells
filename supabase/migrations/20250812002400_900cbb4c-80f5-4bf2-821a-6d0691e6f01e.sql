-- 1) Allow public reading of users for Employee page visibility
DO $$ BEGIN
  -- Create a permissive SELECT policy if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'users' AND policyname = 'Allow all users to view users'
  ) THEN
...
-- 2) Extend add_cash_transaction to accept optional created_by when no Supabase Auth
