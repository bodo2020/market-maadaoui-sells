alter table public.customer_interactions add column if not exists branch_id uuid references public.branches(id) on delete set null;
alter table public.customer_interactions add column if not exists assigned_to uuid references auth.users(id) on delete set null;
alter table public.customer_interactions add column if not exists completed_at timestamptz;
alter table public.customer_interactions add column if not exists completed_by uuid references auth.users(id) on delete set null;

alter table public.customer_interactions drop constraint if exists customer_interactions_type_check;
alter table public.customer_interactions add constraint customer_interactions_type_check check (type in ('call','email','meeting','note','whatsapp'));

create index if not exists customer_interactions_pending_schedule_idx on public.customer_interactions(status,scheduled_at) where status='pending';
create index if not exists customer_interactions_branch_schedule_idx on public.customer_interactions(branch_id,scheduled_at) where status='pending';
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_super boolean; v_customer_id uuid; v_status text; v_outcome text:=btrim(coalesce(p_outcome,''));
begin
  if auth.uid() is null then raise exception using errcode='42501',message='AUTH_REQUIRED'; end if;
  v_super:=private.staff_is_super_admin(auth.uid());
  if not v_super and (p_branch_id is null or not public.staff_has_permission('customers.manage',p_branch_id)) then raise exception using errcode='42501',message='CUSTOMER_MANAGE_DENIED'; end if;
  if length(v_outcome)<2 then raise exception using errcode='22023',message='FOLLOWUP_OUTCOME_REQUIRED'; end if;
  select customer_id,status into v_customer_id,v_status from public.customer_interactions where id=p_interaction_id and type in ('call','email','meeting','whatsapp') for update;
  ...
  insert into public.customer_admin_audit(customer_id,action_type,branch_id,created_by,metadata) values(v_customer_id,'followup_completed',p_branch_id,auth.uid(),jsonb_build_object('interaction_id',p_interaction_id,'outcome',v_outcome));
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_super boolean; v_customer_id uuid; v_status text; v_reason text:=btrim(coalesce(p_reason,''));
begin
  if auth.uid() is null then raise exception using errcode='42501',message='AUTH_REQUIRED'; end if;
  v_super:=private.staff_is_super_admin(auth.uid());
  if not v_super and (p_branch_id is null or not public.staff_has_permission('customers.manage',p_branch_id)) then raise exception using errcode='42501',message='CUSTOMER_MANAGE_DENIED'; end if;
  if length(v_reason)<2 then raise exception using errcode='22023',message='FOLLOWUP_CANCEL_REASON_REQUIRED'; end if;
  select customer_id,status into v_customer_id,v_status from public.customer_interactions where id=p_interaction_id for update;
  if v_customer_id is null then raise exception using errcode='22023',message='FOLLOWUP_NOT_FOUND'; end if;
  if v_status<>'pending' then raise exception using errcode='22023',message='FOLLOWUP_ALREADY_CLOSED'; end if;
  ...
  insert into public.customer_admin_audit(customer_id,action_type,branch_id,created_by,metadata) values(v_customer_id,'followup_cancelled',p_branch_id,auth.uid(),jsonb_build_object('interaction_id',p_interaction_id,'reason',v_reason));
  return public.get_customer_management_workspace(v_customer_id,p_branch_id);
end $$;
