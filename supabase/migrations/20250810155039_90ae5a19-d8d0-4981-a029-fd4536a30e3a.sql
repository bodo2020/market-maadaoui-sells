...
-- Policies: everyone can read branches; only admins can modify
CREATE POLICY IF NOT EXISTS "Branches are viewable by everyone"
ON public.branches FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Admins can manage branches"
ON public.branches FOR ALL USING (is_admin() OR is_super_admin()) WITH CHECK (is_admin() OR is_super_admin());
...
CREATE TABLE IF NOT EXISTS public.user_branch_roles (
  ...
  role text NOT NULL DEFAULT 'branch_manager', -- aligns with existing app roles
  created_at timestamptz DEFAULT now(),
...
CREATE POLICY IF NOT EXISTS "Admins manage user_branch_roles"
ON public.user_branch_roles FOR ALL
USING (is_admin() OR is_super_admin())
WITH CHECK (is_admin() OR is_super_admin());

CREATE POLICY IF NOT EXISTS "Users can view their branch roles"
ON public.user_branch_roles FOR SELECT
USING (user_id = auth.uid());

-- Helper: check if a user has access to a branch
CREATE OR REPLACE FUNCTION public.has_branch_access(_user uuid, _branch uuid)
RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = ''
AS $$
  SELECT coalesce((SELECT is_admin() OR is_super_admin()), false)
         OR EXISTS (
...
-- 3) Per-branch inventory: add branch_id to inventory
ALTER TABLE public.inventory ADD COLUMN IF NOT EXISTS branch_id uuid;

-- Ensure main branch id
DO $$
DECLARE v_main uuid;
BEGIN
  ...
  END IF;
  -- backfill existing inventory rows
  UPDATE public.inventory SET branch_id = v_main WHERE branch_id IS NULL;
END $$;

-- Set NOT NULL and uniqueness per product+branch
...
-- 3.a) Trigger: keep products.quantity as SUM of per-branch quantities
...
DO $$
DECLARE v_main uuid; exist boolean;
BEGIN
  SELECT id INTO v_main FROM public.branches WHERE code = 'MAIN' OR name = 'الفرع الرئيسي' LIMIT 1;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='purchases') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='purchases' AND column_name='branch_id') THEN
      ...
      EXECUTE 'UPDATE public.purchases SET branch_id = '''||v_main||''' WHERE branch_id IS NULL';
...
CREATE POLICY IF NOT EXISTS "Branch users can view related transfers"
...
CREATE POLICY IF NOT EXISTS "Branch managers/admins can manage transfers"
ON public.inventory_transfers FOR ALL
USING (is_admin() OR is_super_admin() OR public.has_branch_access(auth.uid(), from_branch_id) OR public.has_branch_access(auth.uid(), to_branch_id))
WITH CHECK (is_admin() OR is_super_admin() OR public.has_branch_access(auth.uid(), from_branch_id) OR public.has_branch_access(auth.uid(), to_branch_id));

CREATE POLICY IF NOT EXISTS "Branch users can view transfer items via parent"
...
CREATE POLICY IF NOT EXISTS "Branch managers/admins manage transfer items"
ON public.inventory_transfer_items FOR ALL
USING (EXISTS (
  SELECT 1 FROM public.inventory_transfers t WHERE t.id = transfer_id AND (
    is_admin() OR is_super_admin() OR public.has_branch_access(auth.uid(), t.from_branch_id) OR public.has_branch_access(auth.uid(), t.to_branch_id)
  )
))
WITH CHECK (EXISTS (
  SELECT 1 FROM public.inventory_transfers t WHERE t.id = transfer_id AND (
...
-- 6) Optional views or indexes could be added here
