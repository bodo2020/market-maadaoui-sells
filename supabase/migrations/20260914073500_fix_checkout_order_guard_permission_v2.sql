...
begin
  ...
  if tg_op='DELETE' then
    if auth.uid() is null or not public.staff_has_permission('online_orders.manage',old.branch_id) then
  ...
  if auth.uid() is null or not public.staff_has_permission('online_orders.manage',coalesce(new.branch_id,old.branch_id)) then
    raise exception using errcode='42501',message='ORDER_MANAGER_REQUIRED';
  end if;
  ...
  if new.status='cancelled' and old.status<>'cancelled' then
    ...
    for x in
      select value
      from jsonb_array_elements(old.stock_deductions)
      order by value->>'branch_id',value->>'product_id'
    loop
