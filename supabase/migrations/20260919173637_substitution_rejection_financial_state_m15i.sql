-- M15i — use an allowed financial state when a substitution is rejected.
...
begin
  ...
  if new.status='rejected' then
    update private.order_fulfillment_substitutions_v1
      set financial_state='waived',updated_at=now(),metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('financial_settlement_state','waived')
      where id=new.id;
    return new;
  end if;
