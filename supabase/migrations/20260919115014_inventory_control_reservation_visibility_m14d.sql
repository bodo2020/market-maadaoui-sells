-- M14d — Admin Inventory Control exposes on-hand / reserved / available
CREATE OR REPLACE FUNCTION public.get_inventory_control_center_v2(p_branch_id uuid, p_search text DEFAULT NULL::text, p_status text DEFAULT 'all'::text, p_category_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  with sold as (
    ...
    select inv.product_id,inv.quantity,
      private.inventory_reserved_quantity_v1(v_inventory_branch,inv.product_id)::numeric reserved_quantity,
      coalesce(inv.min_stock_level,0)::numeric min_stock_level,
    ...
    select *,
      greatest(quantity-reserved_quantity,0)::numeric available_quantity,
      case when net_sold_30d>0 and quantity>0 then quantity/(net_sold_30d/30.0) else null end::numeric days_cover
    from base
  )
  select jsonb_build_object(
    ...
    'reserved_measure',round(coalesce(sum(greatest(reserved_quantity,0)),0),3),
    'available_measure',round(coalesce(sum(greatest(available_quantity,0)),0),3),
    'reserved_sku_rows',count(*) filter(where reserved_quantity>0)::bigint,
    'fully_reserved_rows',count(*) filter(where quantity>0 and available_quantity<=0)::bigint,
  ...
  with sold as (
    ...
    select inv.product_id,inv.quantity,
      private.inventory_reserved_quantity_v1(v_inventory_branch,inv.product_id)::numeric reserved_quantity,
      coalesce(inv.min_stock_level,0)::numeric min_stock_level,inv.max_stock_level::numeric max_stock_level,inv.alert_enabled,
  ...
  select (select count(*) from filtered)::bigint,
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'reserved_quantity',round(reserved_quantity,3),
      'available_quantity',round(available_quantity,3),
      'unit_of_measure',unit_of_measure,'shelf_location',shelf_location,
