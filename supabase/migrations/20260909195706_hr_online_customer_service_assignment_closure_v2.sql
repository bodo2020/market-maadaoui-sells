...
begin
  ...
  with order_events as (
    ...
    select count(*)::int samples,
           round(avg(extract(epoch from (response_at-order_created_at))/60.0)::numeric,1) avg_minutes
    from first_responses
    where response_at>=order_created_at
  ), prep_samples as (
    ...
    select count(*)::int samples,
           round(avg(extract(epoch from (ready_at-preparing_at))/60.0)::numeric,1) avg_minutes
    from prep_samples
  ...
  select jsonb_build_object(
    ...
    'orders',jsonb_build_object(
      ...
      'sla',jsonb_build_object('enabled',false,'rate',null,'reason','online_order_sla_policy_not_configured')
    ),
    ...
    'notes',jsonb_build_array(
      ...
      'SLA الطلبات الإلكترونية غير محسوب حتى يتم تعريف سياسة SLA فعلية للفرع.',
