...
begin
  ...
  select coalesce(sum(r.total_net) filter (where r.status='locked'),0)
  into v_locked_payroll
  from private.hr_payroll_runs r
  ...
  where t.branch_id=p_branch_id
    and t.source_kind in ('hr_treasury_payout','hr_treasury_payroll');
  ...
  from (
    select jsonb_build_object(
      ...
      'principal_amount',round(a.principal_amount,2),
      'outstanding_amount',round(a.outstanding_amount,2),
      'monthly_deduction',round(a.monthly_deduction,2),
      'repayment_months',a.repayment_months,
      'status',a.status,
      'paid_at',a.paid_at,
      'payout_source_kind',a.payout_source_kind,
      'payout_account_name',a.payout_account_name_snapshot,
      'payout_responsible_user_id',a.payout_responsible_user_id,
      'payout_responsible_user_name',ru.name,
    ...
    coalesce(a.paid_at,a.created_at) as sort_at
    from private.hr_salary_advances a
  ...
  from (
    select jsonb_build_object(
      ...
      'payment_source_kind',r.payment_source_kind,
      'payment_account_name',r.payment_account_name_snapshot,
      'payment_responsible_user_id',r.payment_responsible_user_id,
      'payment_responsible_user_name',ru.name,
      'payment_reference',r.payment_reference,
      'payment_requested_at',r.payment_requested_at,
      'paid_at',r.paid_at,
      'delegated_task_id',r.payment_delegated_task_id,
      'delegated_task_status',t.status
    ) as obj,
    coalesce(r.paid_at,r.payment_requested_at,r.created_at) as sort_at
    from private.hr_payroll_runs r
    left join public.users ru on ru.id=r.payment_responsible_user_id
    left join public.operations_tasks t on t.id=r.payment_delegated_task_id
    where r.branch_id=p_branch_id
    order by coalesce(r.paid_at,r.payment_requested_at,r.created_at) desc
    limit 24
  ) x;
  ...
  from (
    select jsonb_build_object(
      ...
      'status',t.status,
      'priority',t.priority,
      'responsible_user_id',t.claimed_by,
      'responsible_user_name',u.name,
  ...
  v_summary := jsonb_build_object(
    ...
    'attention_count',v_unlinked_count+v_failed_tasks
  );
  ...
  return jsonb_build_object(
    ...
    'summary',v_summary,
    'treasury',v_treasury,
    'salary_advances',v_advances,
    'payroll_runs',v_payroll,
    'treasury_tasks',v_tasks,
    'generated_at',now()
  );
end;
$$;
