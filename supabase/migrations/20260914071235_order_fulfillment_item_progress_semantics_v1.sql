...
begin
  select count(*),
         count(*) filter(where picked_quantity+shortage_quantity+substitution_quantity >= required_quantity-0.0005),
         count(*) filter(where shortage_quantity>0),
         count(*) filter(where substitution_quantity>0)
  into v_total,v_resolved,v_short,v_sub
  from private.order_fulfillment_items_v1
  where order_id=p_order_id;

  update private.order_fulfillment_state_v1
  set items_total=coalesce(v_total,0),
      items_picked=coalesce(v_resolved,0),
...
begin
  ...
  select jsonb_build_object(
    ...
    'resolved_count',v_f.items_picked,
    'items',coalesce(jsonb_agg(jsonb_build_object(
      ...
      'note',i.note
    ) order by case when i.picked_quantity+i.shortage_quantity+i.substitution_quantity < i.required_quantity-0.0005 then 0 else 1 end,i.line_no),'[]'::jsonb)
  ) into v_result
...
create or replace function private.normalize_fulfillment_item_status_v1()
returns trigger
language plpgsql
security definer
set search_path=''
as $function$
begin
  if new.picked_quantity+new.shortage_quantity+new.substitution_quantity >= new.required_quantity-0.0005 then
    if new.substitution_quantity>0 then new.status:='substituted';
    elsif new.shortage_quantity>0 then new.status:='shortage';
    else new.status:='picked'; end if;
    new.completed_at:=coalesce(new.completed_at,now());
  elsif new.picked_quantity+new.shortage_quantity+new.substitution_quantity>0 then
    new.status:='picking';
...
revoke all on function private.normalize_fulfillment_item_status_v1() from public,anon,authenticated;
drop trigger if exists normalize_fulfillment_item_status_v1 on private.order_fulfillment_items_v1;
create trigger normalize_fulfillment_item_status_v1
before insert or update of picked_quantity,shortage_quantity,substitution_quantity,required_quantity
on private.order_fulfillment_items_v1
for each row execute function private.normalize_fulfillment_item_status_v1();
