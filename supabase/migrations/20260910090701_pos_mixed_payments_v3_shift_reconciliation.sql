...
create or replace function public.get_my_pos_cash_summary_v3(p_device_id uuid,p_device_token text)
returns jsonb
language plpgsql
stable
security definer
set search_path=''
as $$
declare v_base jsonb; v_preview jsonb; v_breakdown jsonb:='[]'::jsonb;
begin
  v_base:=public.get_my_pos_cash_summary(p_device_id,p_device_token);
  if nullif(v_base->>'shift_id','') is null then return v_base||jsonb_build_object('payment_summary_version',3); end if;
  v_preview:=private.pos_shift_reconciliation_preview((v_base->>'shift_id')::uuid);
  select coalesce(jsonb_agg(jsonb_build_object(
    'code',m->>'code','name',m->>'name','method_type',m->>'method_type','settlement_account_id',nullif(m->>'settlement_account_id','')::uuid,
    'sale_count',coalesce((m->>'sale_count')::bigint,0),'base_amount',coalesce((m->>'base_amount')::numeric,0),
    'charged_amount',coalesce((m->>'charged_amount')::numeric,0),
    'fee_amount',coalesce((m->>'customer_fee_amount')::numeric,0)+coalesce((m->>'merchant_fee_amount')::numeric,0),
    'customer_fee_amount',coalesce((m->>'customer_fee_amount')::numeric,0),'merchant_fee_amount',coalesce((m->>'merchant_fee_amount')::numeric,0),
    'pending_refund_amount',coalesce((m->>'pending_refund_amount')::numeric,0),'confirmed_refund_amount',coalesce((m->>'confirmed_refund_amount')::numeric,0),
    'net_shift_amount',coalesce((m->>'expected_amount')::numeric,0),'account_balance',case when m->>'account_balance' is null then null else (m->>'account_balance')::numeric end
  ) order by case when m->>'method_type'='cash' then 0 else 1 end,m->>'name'),'[]'::jsonb)
  into v_breakdown from jsonb_array_elements(v_preview->'methods') m;
  return v_base||jsonb_build_object('payment_summary_version',3,'payment_breakdown',v_breakdown);
end;
$$;

revoke all on function public.get_my_pos_cash_summary_v3(uuid,text) from public,anon;
grant execute on function public.get_my_pos_cash_summary_v3(uuid,text) to authenticated,service_role;
