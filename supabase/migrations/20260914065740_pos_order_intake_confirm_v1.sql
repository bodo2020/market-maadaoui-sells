...
begin
  ...
  if old.status='cancelled' and new.status<>'cancelled' then raise exception using errcode='22023',message='CANCELLED_ORDER_FINAL'; end if;

  if new.status='cancelled' and old.status<>'cancelled' then
    if old.status in ('shipped','delivered') then raise exception using errcode='22023',message='USE_RETURN_PROCESS'; end if;
...
begin
  ...
  if v_order.branch_id is null then raise exception using errcode='22023',message='ORDER_BRANCH_REQUIRED'; end if;
  ...
  if v_order.status::text='confirmed' then return jsonb_build_object('ok',true,'idempotent',true,'order_id',v_order.id,'status','confirmed'); end if;
  if v_order.status::text<>'pending' then raise exception using errcode='40001',message='ORDER_STATUS_CHANGED'; end if;
  update public.online_orders set status='confirmed'::public.order_status,updated_at=now() where id=v_order.id returning * into v_order;
  return jsonb_build_object('ok',true,'order_id',v_order.id,'status',v_order.status::text,'confirmed_at',v_order.updated_at);
end;
$function$;

revoke all on function public.accept_pos_online_order_v1(uuid) from public,anon;
grant execute on function public.accept_pos_online_order_v1(uuid) to authenticated;
