...
begin
  ...
  if v_shift.id is not null then
    ...
    with payment_rows as (
      ...
      select legacy.code,legacy.name,legacy.method_type,1::bigint,legacy.amount,legacy.amount,0::numeric,0::numeric,0::numeric
      from public.sales s
      cross join lateral (
      ...
      select code,name,method_type,sum(sale_count)::bigint sale_count,sum(base_amount)::numeric base_amount,
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      'code',code,'name',name,'method_type',method_type,'sale_count',sale_count,
      'base_amount',base_amount,'charged_amount',charged_amount,'fee_amount',fee_amount,
      'customer_fee_amount',customer_fee_amount,'merchant_fee_amount',merchant_fee_amount
    ) order by case when method_type='cash' then 0 else 1 end,name),'[]'::jsonb)
