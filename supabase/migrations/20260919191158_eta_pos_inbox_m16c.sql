-- M16c — expose ETA v2 fields to POS order inbox.
    ...
    'orders',coalesce((
      ...
      from (
        select jsonb_build_object(
          ...
          'eta_confidence',coalesce(e.confidence,'low'),
          'eta_model_version',e.model_version,
          'eta_live_at',e.live_eta,
          'eta_live_window_start',e.live_window_start,
          'eta_live_window_end',e.live_window_end,
          'eta_remaining_minutes',coalesce((e.components->>'remaining_minutes')::integer,0),
          'eta_ready_remaining_minutes',coalesce((e.components->>'ready_remaining_minutes')::integer,0),
          'eta_route_minutes',coalesce((e.components->>'road_minutes')::integer,0),
          'driver_name',du.name,
