...
begin
  ...
  if p_employee_id is null or p_branch_id is null then
    raise exception 'employee_and_branch_required';
  end if;
  ...
  if not v_is_super
     and not exists (
       ...
       where ubr.user_id = p_employee_id
  ...
  with count_rows as (
    ...
    select
      ...
      count(*) filter (where status = 'discrepancy')::int discrepancy,
      ...
      round(coalesce(avg(extract(epoch from (coalesce(completed_at, submitted_at) - coalesce(started_at, assigned_at))) / 60.0) filter (where submitted_at is not null), 0)::numeric, 1) avg_active_minutes,
  ...
  select jsonb_build_object(
    ...
    'notes', jsonb_build_array(
      ...
      'تأكيد Peer Recount يعني أن موظفًا آخر أعاد العد ووصل لنفس الكمية الفعلية للعد الأول.',
      'متوسط زمن العد يقيس زمن النشاط من بدء المهمة حتى الإكمال عندما يكون وقت البدء متاحًا، وإلا يستخدم وقت الإسناد.'
    )
  ) into v_result
  from public.users u
