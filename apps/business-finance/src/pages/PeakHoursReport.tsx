...
import { ArrowRight, CalendarDays, Clock3, Download, Printer, RefreshCcw } from 'lucide-react';
...
import { fetchPeakHoursReport } from '../services/businessAnalytics';
...
export default function PeakHoursReport() {
  ...
  async function load() {
    ...
    setLoading(true);
    setError(null);
    try { setData(await fetchPeakHoursReport(filters)); }
    catch (cause) { setData(null); setError(cause instanceof Error ? cause.message : 'تعذر تحميل ساعات الذروة.'); }
  ...
  function exportCsv() {
    ...
    const lines = [
      ['الساعة', 'فواتير الفرع', 'مبيعات الفرع', 'طلبات الأونلاين', 'طلبات مسلمة', 'قيمة طلبات الأونلاين', 'إجمالي النشاط'],
      ...hourly.map((row) => [hourLabel(row.hour), num(row.pos_transactions), num(row.pos_sales), num(row.online_orders), num(row.online_delivered_orders), num(row.online_order_value), num(row.combined_activity)]),
    ];
  ...
  return <div className="stack-lg sales-report-page">
    ...
    <section className="section-card analytics-guidance"><div><span className="eyebrow">Staffing Signal</span><h3>كيف تقرأ التقرير؟</h3></div><p>ذروة الفرع مبنية على وقت إصدار فواتير POS، وذروة الأونلاين مبنية على وقت وصول الطلب. نعرض الطلبات المسلّمة منفصلة، لكن لا نفترض مدة توصيل من بيانات تاريخية غير مكتملة.</p></section>
  </div>;
}
