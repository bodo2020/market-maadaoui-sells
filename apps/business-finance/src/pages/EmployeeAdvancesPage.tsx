...
import { ArrowRight, Banknote, CheckCircle2, CircleDollarSign, Landmark, Plus, ReceiptText, RefreshCcw, RotateCcw, Upload, WalletCards, X, XCircle } from 'lucide-react';
...
export default function EmployeeAdvancesPage() {
  ...
  async function load() {
    ...
    try { setWorkspace(await fetchEmployeeAdvanceWorkspace(selectedBranch.branch_id)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'تعذر تحميل العهد والسلف.'); }
  ...
  async function decide(document: EmployeeAdvanceDocument, decision: 'approve' | 'reject') {
    ...
    try {
      ...
      await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'تعذر تنفيذ القرار.'); }
  ...
  return <div className="stack-lg expense-page">
    <section className="page-intro expense-page__intro">
      <div><span className="eyebrow">Employee Advances</span><h2>العهد والسلف</h2><p>صرف العهدة ليس مصروف تشغيل. المصروف يدخل الربحية فقط عند تسويته بإثبات، وأي باقي يُرد إلى الخزنة أو الحساب المالي كحركة سيولة مستقلة.</p></div>
      <div className="expense-policy-actions"><Link className="secondary-button" to="/finance/expenses"><ArrowRight size={17}/> المصروفات</Link>{workspace?.permissions.can_manage && <button className="primary-button" onClick={() => setDialog({ kind: 'create' })}><Plus size={17}/> عهدة جديدة</button>}</div>
    </section>
    ...
    <section className="expense-list">
      {loading ? [1,2].map((item) => <article className="expense-card expense-card--loading" key={item}><div className="skeleton wide"/><div className="skeleton medium"/></article>) : !workspace?.documents.length ? <article className="section-card empty-data"><span>—</span><p>لا توجد عهد أو سلف في هذا الفرع.</p></article> : workspace.documents.map((document) => <article className="expense-card" key={document.id}>
        <header className="expense-card__head"><div className="expense-card__identity"><div className="expense-card__icon"><Landmark size={20}/></div><div><strong>{document.document_number}</strong><span>{document.employee_name}</span></div></div><AdvanceStatus document={document}/></header>
        ...
        {document.settlements.length > 0 && <div className="expense-payments"><strong>حركات التسوية</strong>{document.settlements.filter((settlement) => settlement.status === 'active').map((settlement) => <div className="expense-payment-row" key={settlement.id}><div><span>{settlement.settlement_type === 'expense_receipt' ? 'مصروف مثبت' : 'رد متبقي'} · {money(settlement.amount)}</span><small>{new Date(settlement.settled_at).toLocaleString('ar-EG')} · {settlement.description}</small></div>{settlement.receipt_url && <SettlementReceipt value={settlement.receipt_url}/>}</div>)}</div>}
    ...
    {dialog?.kind === 'create' && workspace && selectedBranch && <CreateAdvanceDialog workspace={workspace} branchId={selectedBranch.branch_id} onClose={() => setDialog(null)} onSaved={async () => { setDialog(null); setNotice('تم إنشاء طلب العهدة وإرساله لدورة الاعتماد.'); await load(); }} onError={setError}/>} 
    ...
    {dialog?.kind === 'expense' && workspace && selectedBranch && <SettleExpenseDialog document={dialog.document} workspace={workspace} branchId={selectedBranch.branch_id} onClose={() => setDialog(null)} onSaved={async () => { setDialog(null); setNotice('تم تسجيل المصروف المثبت وتسوية جزء من العهدة.'); await load(); }} onError={setError}/>} 
...
function DisburseDialog({ document, sources, onClose, onSaved, onError }: { document: EmployeeAdvanceDocument; sources: ExpensePayoutSource[]; onClose: () => void; onSaved: () => void; onError: (message: string | null) => void }) {
  const [sourceId, setSourceId] = useState(sources[0]?.account_id || ''); const [amount, setAmount] = useState(String(document.undisbursed_amount)); const [fee, setFee] = useState('0'); const [reference, setReference] = useState(''); const [note, setNote] = useState(''); const [saving, setSaving] = useState(false);
  async function submit(event: FormEvent) { event.preventDefault(); const source = sources.find((item) => item.account_id === sourceId); const numeric = Number(amount); if (!source || !Number.isFinite(numeric) || numeric <= 0 || numeric > document.undisbursed_amount) return; setSaving(true); onError(null); try { await disburseEmployeeAdvance({ documentId: document.id, source, amount: numeric, actualFee: Number(fee || 0), providerReference: reference.trim(), note: note.trim() }); onSaved(); } catch (cause) { onError(cause instanceof Error ? cause.message : 'تعذر صرف العهدة.'); } finally { setSaving(false); } }
...
function SettleExpenseDialog({ document, workspace, branchId, onClose, onSaved, onError }: { document: EmployeeAdvanceDocument; workspace: EmployeeAdvanceWorkspace; branchId: string; onClose: () => void; onSaved: () => void; onError: (message: string | null) => void }) {
  const [categoryId, setCategoryId] = useState(workspace.expense_categories[0]?.id || ''); const [amount, setAmount] = useState(String(document.advance_outstanding)); const [description, setDescription] = useState(''); const [invoice, setInvoice] = useState(''); const [note, setNote] = useState(''); const [file, setFile] = useState<File | null>(null); const [fileError, setFileError] = useState<string | null>(null); const [saving, setSaving] = useState(false);
...
function ReturnAdvanceDialog({ document, sources, onClose, onSaved, onError }: { document: EmployeeAdvanceDocument; sources: ExpensePayoutSource[]; onClose: () => void; onSaved: () => void; onError: (message: string | null) => void }) {
  const [sourceId, setSourceId] = useState(sources[0]?.account_id || ''); const [amount, setAmount] = useState(String(document.advance_outstanding)); const [fee, setFee] = useState('0'); const [reference, setReference] = useState(''); const [note, setNote] = useState(''); const [saving, setSaving] = useState(false);
  async function submit(event: FormEvent) { event.preventDefault(); const source = sources.find((item) => item.account_id === sourceId); const numeric = Number(amount); if (!source || !Number.isFinite(numeric) || numeric <= 0 || numeric > document.advance_outstanding) return; setSaving(true); onError(null); try { await settleEmployeeAdvanceReturn({ documentId: document.id, source, amount: numeric, providerReference: reference.trim(), note: note.trim() }); onSaved(); } catch (cause) { onError(cause instanceof Error ? cause.message : 'تعذر رد متبقي العهدة.'); } finally { setSaving(false); } }
...
function SettlementReceipt({ value }: { value: string }) {
  ...
  async function open() { setOpening(true); const popup = window.open('about:blank','_blank'); try { const url = await resolveExpenseReceiptUrl(value); if (!url) throw new Error('الإثبات غير متاح.'); if (popup) popup.location.href = url; else window.location.href = url; } catch (cause) { popup?.close(); window.alert(cause instanceof Error ? cause.message : 'تعذر فتح الإثبات.'); } finally { setOpening(false); } }
