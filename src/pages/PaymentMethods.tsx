...
const typeIcon = (type: POSPaymentMethodType) => {
  if (type === "cash") return Banknote;
  if (type === "card") return CreditCard;
  if (type === "digital_wallet") return Smartphone;
  if (type === "bank_transfer") return Landmark;
  return WalletCards;
};
      ...
      <div dir="rtl" className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
        <div className="overflow-hidden rounded-3xl bg-[#005931] p-5 text-white shadow-sm md:p-7">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-50">تحكم في وسائل الدفع المتاحة داخل نقطة البيع ورسوم كل خدمة بدون تعديل كود الـPOS.</p>
            </div>
        ...
        {!currentBranchId ? (
          ...
          <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            {methods.map(method => {
              const Icon = typeIcon(method.method_type);
              ...
              return (
                <Card key={method.id} className={!method.active ? "opacity-60" : ""}>
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-50 text-[#005931]"><Icon className="h-5 w-5" /></div>
                        <div><CardTitle className="text-lg">{method.name}</CardTitle><div className="mt-1 text-xs text-muted-foreground">{paymentMethodTypeLabel(method.method_type)} · {method.code}</div></div>
                      </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button variant="outline" className="w-full" onClick={() => setEditing({ ...method })}>تعديل الإعدادات</Button>
                  </CardContent>
                </Card>
              );
            })}
        ...
        <Alert className="border-emerald-200 bg-emerald-50/60">
          <AlertDescription className="leading-6 text-emerald-950">رسوم وسيلة الدفع لا تدخل في قيمة المنتجات ولا نقاط الولاء. لو الرسوم على العميل تُضاف للمبلغ المطلوب تحصيله فقط، ولو على المنشأة تُخصم من صافي الربح بعد الدفع.</AlertDescription>
        </Alert>
      </div>

      <Dialog open={Boolean(editing)} onOpenChange={open => !open && setEditing(null)}>
        <DialogContent dir="rtl" className="max-h-[92vh] overflow-y-auto sm:max-w-xl">
          <DialogHeader><DialogTitle>{editing?.id ? "تعديل وسيلة الدفع" : "إضافة وسيلة دفع"}</DialogTitle><DialogDescription>الإعدادات تُطبق على الفرع الحالي فقط.</DialogDescription></DialogHeader>
          {editing && (
            <div className="space-y-5">
              ...
              <div className="space-y-2"><Label>النوع</Label><Select value={editing.method_type} onValueChange={value => setEditing({ ...editing, method_type: value as POSPaymentMethodType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="cash">نقدي</SelectItem><SelectItem value="card">بطاقة بنكية</SelectItem><SelectItem value="digital_wallet">محفظة رقمية</SelectItem><SelectItem value="bank_transfer">تحويل بنكي</SelectItem><SelectItem value="other">وسيلة أخرى</SelectItem></SelectContent></Select></div>
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-2"><Label>نوع الرسوم</Label><Select value={editing.fee_type} onValueChange={value => setEditing({ ...editing, fee_type: value as POSPaymentFeeType, fee_value: value === "none" ? 0 : editing.fee_value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">بدون رسوم</SelectItem><SelectItem value="percent">نسبة مئوية</SelectItem><SelectItem value="fixed">مبلغ ثابت</SelectItem></SelectContent></Select></div>
              ...
              <div className="space-y-2"><Label>من يتحمل الرسوم؟</Label><Select value={editing.fee_bearer} onValueChange={value => setEditing({ ...editing, fee_bearer: value as POSPaymentFeeBearer })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="business">المنشأة</SelectItem><SelectItem value="customer">العميل</SelectItem></SelectContent></Select></div>
              {preview && editing.fee_type !== "none" && editing.fee_value > 0 && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm"><div className="font-bold text-[#005931]">معاينة على فاتورة 1000 ج.م</div><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><span>قيمة الرسوم</span><strong>{preview.fee.toFixed(2)} ج.م</strong><span>المطلوب من العميل</span><strong>{preview.amountCharged.toFixed(2)} ج.م</strong><span>صافي التسوية المتوقع</span><strong>{preview.estimatedNetSettlement.toFixed(2)} ج.م</strong></div></div>}
              ...
              <Button className="h-12 w-full bg-[#005931] hover:bg-[#004a29]" disabled={mutation.isPending} onClick={save}>{mutation.isPending && <RefreshCw className="ml-2 h-4 w-4 animate-spin" />}حفظ وسيلة الدفع</Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
