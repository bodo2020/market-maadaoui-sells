...
import FranchiseFinancePanel from "@/components/franchise/FranchiseFinancePanel";
...
export default function FranchiseDetailsPage() {
  ...
  const transitionMutation = useMutation({
    mutationFn: ({ action: nextAction, reason: transitionReason }: { action: FranchiseAgreementAction; reason?: string | null }) =>
      transitionFranchiseAgreement(merchantId!, nextAction, transitionReason),
    onSuccess: async () => {
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ["franchise-finance", merchantId] }),
      ]);
    },
  ...
  const branchMutation = useMutation({
    mutationFn: () =>
      createFranchiseBranch(merchantId!, {
        name: branch.name,
  ...
  const submitTransition = () => {
    ...
    if ((action === "suspend" || action === "terminate") && !reason.trim()) {
      toast.error("سبب الإيقاف أو الإنهاء مطلوب.");
      return;
    }
      ...
      <MainLayout>
        <div className="grid min-h-[420px] place-items-center">
          <div className="text-center font-bold text-slate-500">
            <RefreshCw className="mx-auto mb-3 h-6 w-6 animate-spin text-[#005931]" />
            جارٍ تحميل Franchise 360…
          </div>
        </div>
      </MainLayout>
    );
  ...
  if (query.error || !query.data || !agreement || !merchantId) {
    return (
      <MainLayout>
        <div className="rounded-2xl border border-red-200 bg-red-50 p-6 font-bold text-red-800">
          {query.error instanceof Error ? query.error.message : "تعذر تحميل بيانات الـFranchise"}
        </div>
      </MainLayout>
    );
  }
    ...
    <MainLayout>
      <div dir="rtl" className="space-y-6 pb-12">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <button
              ...
              className="mb-3 inline-flex items-center gap-2 text-xs font-black text-[#005931]"
            >
              <ArrowRight className="h-4 w-4" />
              مركز الفروع والشبكة
            </button>
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">FRANCHISE 360</Badge>
              <Badge variant="outline" className={statusClasses[agreement.status]}>{statusLabels[agreement.status]}</Badge>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!(["terminated", "expired"] as string[]).includes(agreement.status) ? (
            ...
            {actions.map((item) => (
              <Button
                key={item}
                variant={actionTone[item] || "default"}
                className={item === "approve" || item === "activate" || item === "resume" || item === "submit_review" ? "bg-[#005931] hover:bg-[#004a29]" : ""}
                onClick={() => {
                  setAction(item);
                  setReason("");
                }}
              >
                {item === "activate" || item === "resume" ? <PlayCircle className="ml-2 h-4 w-4" /> : item === "suspend" ? <PauseCircle className="ml-2 h-4 w-4" /> : item === "terminate" ? <XCircle className="ml-2 h-4 w-4" /> : <ShieldCheck className="ml-2 h-4 w-4" />}
                {actionLabels[item]}
              </Button>
            ))}
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[1.05fr_1.4fr]">
          <Card className="border-slate-200 shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg font-black">
                <Building2 className="h-5 w-5 text-[#005931]" />
                الكيان القانوني
              </CardTitle>
            </CardHeader>
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg font-black">
                <CircleDollarSign className="h-5 w-5 text-[#005931]" />
                الشروط المالية
              </CardTitle>
            </CardHeader>
        ...
        <FranchiseFinancePanel merchantId={merchantId} fallbackCurrency={agreement.currency} />

        <Card className="border-slate-200 shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg font-black">
              <FileSignature className="h-5 w-5 text-[#005931]" />
              سياسات الامتياز
            </CardTitle>
          </CardHeader>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-lg font-black">
              <Network className="h-5 w-5 text-[#005931]" />
              فروع المشغل
            </CardTitle>
            <span className="text-xs font-bold text-slate-400">تفعيل القنوات يتم من Branch 360</span>
          </CardHeader>
          <CardContent className="space-y-2">
            {data.branches.map((item) => (
              <button
                key={item.id}
                type="button"
                onClick={() => navigate(`/branches/${item.id}`)}
                className="w-full rounded-2xl border border-slate-100 bg-slate-50 p-4 text-right transition hover:border-emerald-200 hover:bg-emerald-50/40"
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-black text-slate-900">{item.name}</span>
                      <Badge variant="outline" className={item.active ? "border-emerald-200 text-emerald-700" : "border-red-200 text-red-700"}>
                        {item.active ? "نشط" : "متوقف"}
                      </Badge>
                      <span className="text-[11px] font-bold text-slate-400">{item.code}</span>
                    </div>
                    <p className="mt-1 text-xs text-slate-500">{item.address || "لا يوجد عنوان"}</p>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {(Object.entries(item.channels || {}) as [string, BranchChannelRuntime][]).map(([key, runtime]) => (
                      <span
                        key={key}
                        className={`rounded-full border px-2 py-1 text-[10px] font-black ${runtime.effective_enabled ? "border-emerald-200 bg-emerald-50 text-emerald-700" : runtime.configured_enabled ? "border-amber-200 bg-amber-50 text-amber-700" : "border-slate-200 bg-white text-slate-400"}`}
                      >
                        {key}
                      </span>
                    ))}
        ...
        <Card className="border-slate-200 shadow-sm">
          ...
          <CardContent className="space-y-3">
            {data.events.length ? data.events.map((event) => (
              <div key={event.id} className="flex flex-col gap-2 rounded-2xl border border-slate-100 p-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="font-black text-slate-800">{actionLabels[event.event_type as FranchiseAgreementAction] || event.event_type}</p>
                  <p className="mt-1 text-xs text-slate-500">{event.from_status || "—"} → {event.to_status || "—"}{event.reason ? ` • ${event.reason}` : ""}</p>
                </div>
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-400">لا توجد أحداث مسجلة.</div>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={!!action} onOpenChange={(open) => {
        ...
        <DialogContent dir="rtl">
          <DialogHeader>
            <DialogTitle>{action ? actionLabels[action] : "تحديث العقد"}</DialogTitle>
            <DialogDescription>
              {action === "suspend"
                ? "سيتم إيقاف القنوات الفعلية لكل فروع المشغل بدون حذف إعداداتها، ويمكن استعادتها عند الاستئناف."
                : action === "terminate"
                  ? "الإنهاء إجراء قوي: سيصبح المشغل Inactive وستتوقف الفروع وقنوات البيع."
                  : "سيتم تطبيق الانتقال على العقد الحالي والمشغل."}
            </DialogDescription>
          </DialogHeader>
          {action === "suspend" || action === "terminate" ? (
            <div className="space-y-2">
              <Label>السبب *</Label>
              <Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="اكتب سبب واضح للتدقيق والمراجعة" />
            </div>
          ) : null}
          <DialogFooter>
            ...
            <Button
              ...
              className={action !== "terminate" ? "bg-[#005931] hover:bg-[#004a29]" : ""}
              disabled={transitionMutation.isPending}
              onClick={submitTransition}
            >
              {transitionMutation.isPending ? "جارٍ التنفيذ…" : "تأكيد"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={branchOpen} onOpenChange={setBranchOpen}>
        <DialogContent dir="rtl">
          <form onSubmit={submitBranch}>
            <DialogHeader>
              <DialogTitle>إضافة فرع Franchise</DialogTitle>
              <DialogDescription>الفرع الجديد سيبدأ Inactive وكل قنواته OFF، وبعد الإنشاء سيتم فتح Branch 360 لإكمال التشغيل.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-5 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input dir="ltr" value={branch.code} onChange={(event) => setBranch({ ...branch, code: event.target.value })} />
              </div>
              <div className="space-y-2">
                <Label>الهاتف</Label>
                <Input dir="ltr" value={branch.phone} onChange={(event) => setBranch({ ...branch, phone: event.target.value })} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label>العنوان</Label>
              ...
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input dir="ltr" type="email" value={branch.email} onChange={(event) => setBranch({ ...branch, email: event.target.value })} />
              </div>
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setBranchOpen(false)}>إلغاء</Button>
              <Button type="submit" className="bg-[#005931] hover:bg-[#004a29]" disabled={branchMutation.isPending}>
                <Plus className="ml-2 h-4 w-4" />
                {branchMutation.isPending ? "جارٍ الإضافة…" : "إنشاء الفرع"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
