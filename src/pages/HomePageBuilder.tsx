...
const HomePageBuilder = () => {
  ...
  useEffect(() => { void Promise.all([loadBuilder(), loadAnalytics(30)]); }, []);
  useEffect(() => { void loadAnalytics(analyticsDays); }, [analyticsDays]);
  ...
  const analyticsTotals = useMemo(() => {
    ...
    const addToCarts = analytics.reduce((sum, item) => sum + item.add_to_carts, 0);
    const orders = analytics.reduce((sum, item) => sum + item.orders, 0);
    const revenue = analytics.reduce((sum, item) => sum + item.revenue, 0);
    return {
      impressions,
      clicks,
      addToCarts,
      orders,
      revenue,
      ctr: impressions ? Number(((clicks * 100) / impressions).toFixed(2)) : 0,
      cartRate: impressions ? Number(((addToCarts * 100) / impressions).toFixed(2)) : 0,
      conversion: impressions ? Number(((orders * 100) / impressions).toFixed(2)) : 0,
    };
  }, [analytics]);
  ...
  const openEdit = (section: HomeSectionRecord) => {
    ...
    setEditor({
      ...
      experiment_variant: section.experiment_variant || '',
    });
    setEditorOpen(true);
  };
  ...
  const saveEditor = async () => {
    if (!state) return;
    const experimentKey = editor.experiment_key.trim();
    if (editor.section_type === 'product_collection' && !editor.collection_id) {
      toast.error('اختار مجموعة المنتجات'); return;
    }
    if (editor.starts_at && editor.ends_at && new Date(editor.ends_at) <= new Date(editor.starts_at)) {
      toast.error('وقت النهاية لازم يكون بعد وقت البداية'); return;
    }
    if (experimentKey && (experimentKey.length < 3 || experimentKey.length > 80)) {
      toast.error('اسم تجربة A/B لازم يكون من 3 إلى 80 حرف'); return;
    }
    ...
    const patch: Partial<HomeSectionRecord> = {
      ...
      experiment_variant: experimentKey ? (editor.experiment_variant || 'A') : null,
    };
  ...
  const persistOrder = async (next: HomeSectionRecord[]) => {
    ...
    try { await reorderHomeSections(next); }
    catch { toast.error('تعذر حفظ الترتيب'); await loadBuilder(); }
  };
  ...
  const runSmartReorder = async () => {
    if (!state) return;
    try {
      setSmartReordering(true);
      const result = await smartReorderHomeDraft(state.draft.id, analyticsDays);
      if (!result.applied) {
        toast.info(`لسه محتاج بيانات أكتر للترتيب الذكي (${result.impressions || 0}/${result.minimum_impressions || 50} مشاهدة).`);
        return;
      }
      toast.success(result.changed_sections ? `تم تحسين ترتيب ${result.changed_sections} جزء حسب الأداء` : 'الترتيب الحالي هو الأفضل حسب البيانات المتاحة');
      await Promise.all([loadBuilder(), loadAnalytics(analyticsDays)]);
    } catch (error: any) {
      toast.error(error?.message || 'تعذر تنفيذ الترتيب الذكي');
    } finally { setSmartReordering(false); }
  };
  ...
  const publish = async () => {
    ...
    try {
      ...
      await Promise.all([loadBuilder(), loadAnalytics(analyticsDays)]);
    } catch (error: any) { toast.error(error?.message || 'تعذر نشر الصفحة الرئيسية'); }
    finally { setSaving(false); }
  };
    ...
    <MainLayout>
      <div className="container mx-auto max-w-7xl p-4 md:p-6" dir="rtl">
        <div className="mb-6 flex flex-col gap-4 rounded-3xl border bg-gradient-to-l from-emerald-50 to-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="border-emerald-200 text-[#005931]">Smart Merchandising</Badge>
            </div>
            <h1 className="text-2xl font-black text-slate-950 md:text-3xl">إدارة الصفحة الرئيسية</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">رتّب المحتوى واستهدف العملاء، وقِس الإضافة للسلة والطلبات والإيراد الفعلي، واعمل A/B Test قبل ما تعتمد النسخة الأفضل.</p>
          </div>
        ...
        <section className="mb-6 rounded-3xl border bg-white p-4 shadow-sm md:p-5">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div><h2 className="flex items-center gap-2 text-lg font-black"><BarChart3 className="h-5 w-5 text-[#005931]" />أداء الصفحة والمبيعات</h2><p className="mt-1 text-xs text-muted-foreground">المشاهدة تُحسب عند ظهور 25% من الجزء، والطلب يُنسب لآخر تفاعل قوي من الـHome خلال ساعتين.</p></div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
            <div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-semibold text-emerald-800">المشاهدات</div><strong className="mt-2 block text-xl">{analyticsLoading ? '…' : analyticsTotals.impressions.toLocaleString('ar-EG')}</strong></div>
            <div className="rounded-2xl bg-sky-50 p-4"><div className="text-xs font-semibold text-sky-800">النقرات / CTR</div><strong className="mt-2 block text-xl">{analyticsLoading ? '…' : `${analyticsTotals.clicks.toLocaleString('ar-EG')} · ${analyticsTotals.ctr}%`}</strong></div>
            <div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-semibold text-amber-800">إضافة للسلة</div><strong className="mt-2 block text-xl">{analyticsLoading ? '…' : `${analyticsTotals.addToCarts.toLocaleString('ar-EG')} · ${analyticsTotals.cartRate}%`}</strong></div>
            <div className="rounded-2xl bg-violet-50 p-4"><div className="text-xs font-semibold text-violet-800">الطلبات المنسوبة</div><strong className="mt-2 block text-xl">{analyticsLoading ? '…' : analyticsTotals.orders.toLocaleString('ar-EG')}</strong></div>
            <div className="rounded-2xl bg-rose-50 p-4"><div className="text-xs font-semibold text-rose-800">تحويل لطلب</div><strong className="mt-2 block text-xl">{analyticsLoading ? '…' : `${analyticsTotals.conversion}%`}</strong></div>
            <div className="rounded-2xl bg-teal-50 p-4"><div className="text-xs font-semibold text-teal-800">إيراد مؤكد</div><strong className="mt-2 block text-xl">{analyticsLoading ? '…' : money(analyticsTotals.revenue)}</strong></div>
          </div>
        </section>

        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="text-lg font-black">ترتيب الصفحة</h2><p className="text-xs text-muted-foreground">الترتيب الذكي يحرك أجزاء المنتجات فقط بعد 50 مشاهدة على الأقل، ولا يلمس أجزاء A/B Test.</p></div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={() => void runSmartReorder()} disabled={smartReordering || analyticsLoading}><Sparkles className={`ml-2 h-4 w-4 ${smartReordering ? 'animate-spin' : ''}`} />ترتيب ذكي</Button>
        ...
        <div className="space-y-3">
          {sections.map((section, index) => {
            ...
            return (
              <Card key={section.id} draggable onDragStart={() => setDragId(section.id)} onDragEnd={() => setDragId(null)} onDragOver={event => event.preventDefault()} onDrop={() => handleDrop(section.id)} className={`transition ${dragId === section.id ? 'opacity-50 ring-2 ring-primary/30' : ''} ${section.is_active ? '' : 'opacity-65'}`}>
                <CardContent className="flex flex-col gap-4 p-4">
                  <div className="flex flex-col gap-4 md:flex-row md:items-center">
                    <div className="flex items-center gap-3 md:min-w-[330px]">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2"><strong className="truncate text-base">{section.title || meta.defaultTitle}</strong><Badge variant="secondary">{meta.label}</Badge>{section.experiment_key && <Badge className="bg-indigo-100 text-indigo-800 hover:bg-indigo-100">A/B {section.experiment_variant}</Badge>}</div>
                        <p className="mt-1 truncate text-xs text-muted-foreground">{collection ? `المجموعة: ${collection.title}` : meta.description}</p>
                        {section.experiment_key && <p className="mt-1 truncate text-[11px] font-semibold text-indigo-700">تجربة: {section.experiment_key}</p>}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 border-t pt-3 text-xs">
                    ...
                    <Badge variant="outline"><MousePointerClick className="ml-1 h-3.5 w-3.5" />{stats?.clicks || 0} نقرة · {stats?.ctr || 0}%</Badge>
                    <Badge variant="outline">سلة {stats?.add_to_carts || 0} · {stats?.add_to_cart_rate || 0}%</Badge>
                    <Badge variant="outline">طلبات {stats?.orders || 0} · تحويل {stats?.order_conversion_rate || 0}%</Badge>
                    <Badge variant="outline">إيراد {money(stats?.revenue || 0)}</Badge>
                    <Badge className="bg-slate-100 text-slate-800 hover:bg-slate-100">Score {stats?.performance_score || 0}</Badge>
                  </div>
                </CardContent>
              </Card>
            );
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="max-h-[88vh] max-w-2xl overflow-y-auto" dir="rtl">
          <DialogHeader><DialogTitle>{editor.id ? 'تعديل جزء' : 'إضافة جزء جديد'}</DialogTitle><DialogDescription>حدد المحتوى والجمهور والوقت والفروع، ويمكنك إدخاله في تجربة A/B قبل النشر.</DialogDescription></DialogHeader>
          <div className="grid gap-5 py-2">
            <div className="grid gap-2"><Label>نوع الجزء</Label><select value={editor.section_type} onChange={event => { const type = event.target.value as HomeSectionType; setEditor(value => ({ ...value, section_type: type, title: value.id ? value.title : typeMeta[type].defaultTitle, collection_id: type === 'product_collection' ? value.collection_id : '' })); }} className="h-10 rounded-md border bg-background px-3 text-sm" disabled={Boolean(editor.id)}>{(Object.keys(typeMeta) as HomeSectionType[]).map(type => <option key={type} value={type}>{typeMeta[type].label}</option>)}</select></div>
            <div className="grid gap-2 sm:grid-cols-2"><div className="grid gap-2"><Label>العنوان</Label><Input value={editor.title} onChange={event => setEditor(value => ({ ...value, title: event.target.value }))} /></div><div className="grid gap-2"><Label>وصف صغير (اختياري)</Label><Input value={editor.subtitle} onChange={event => setEditor(value => ({ ...value, subtitle: event.target.value }))} /></div></div>

            {editor.section_type === 'product_collection' && <div className="grid gap-2"><Label>مجموعة المنتجات</Label><select value={editor.collection_id} onChange={event => setEditor(value => ({ ...value, collection_id: event.target.value }))} className="h-10 rounded-md border bg-background px-3 text-sm"><option value="">اختر مجموعة…</option>{state.collections.filter(item => item.active !== false).map((item: HomeBuilderCollection) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><Button type="button" variant="link" className="h-auto w-fit p-0" onClick={() => navigate('/product-collections')}>إدارة مجموعات المنتجات</Button></div>}
            {(editor.section_type === 'featured_products' || editor.section_type === 'smart_recommendations') && <div className="grid gap-2"><Label>عدد المنتجات (3 - 24)</Label><Input type="number" min={3} max={24} value={editor.product_limit} onChange={event => setEditor(value => ({ ...value, product_limit: Number(event.target.value) }))} />{editor.section_type === 'smart_recommendations' && <p className="text-xs leading-5 text-muted-foreground">للعميل المسجل: الاقتراحات تعتمد على تسوقه السابق. للزائر أو بدون تاريخ: تظهر المنتجات الأكثر طلبًا.</p>}</div>}
            ...
            <div className="grid gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/40 p-4">
              <div><Label>A/B Test (اختياري)</Label><p className="mt-1 text-xs leading-5 text-muted-foreground">أنشئ جزأين بنفس اسم التجربة، واحد Variant A والثاني B. كل عميل يرى Variant ثابتًا. الأجزاء داخل تجربة لا تدخل في الترتيب الذكي.</p></div>
              <div className="grid gap-3 sm:grid-cols-[1fr_140px]">
                <Input maxLength={80} placeholder="مثال: september-home-offer" value={editor.experiment_key} onChange={event => setEditor(value => ({ ...value, experiment_key: event.target.value, experiment_variant: event.target.value ? (value.experiment_variant || 'A') : '' }))} />
                <select value={editor.experiment_variant} disabled={!editor.experiment_key.trim()} onChange={event => setEditor(value => ({ ...value, experiment_variant: event.target.value as '' | HomeExperimentVariant }))} className="h-10 rounded-md border bg-background px-3 text-sm disabled:opacity-50"><option value="">بدون تجربة</option><option value="A">Variant A</option><option value="B">Variant B</option></select>
              </div>
            </div>

            <div className="grid gap-3"><div><Label>الجمهور المستهدف</Label><p className="text-xs text-muted-foreground">تقدر تجمع أكثر من نوع. اختيار «كل العملاء» يلغي التخصيص.</p></div><div className="grid gap-2 rounded-2xl border p-3 sm:grid-cols-2">{audienceMeta.map(item => { const checked = editor.audience_segments.includes(item.value); return <label key={item.value} className="flex cursor-pointer items-start gap-3 rounded-xl p-2 hover:bg-muted"><Checkbox checked={checked} onCheckedChange={() => toggleAudience(item.value)} /><span><strong className="block text-sm">{item.label}</strong><span className="text-xs text-muted-foreground">{item.description}</span></span></label>; })}</div></div>
            <div className="grid gap-3"><div><Label>الجدولة</Label><p className="text-xs text-muted-foreground">اتركها فارغة ليظهر الجزء دائمًا.</p></div><div className="grid gap-3 sm:grid-cols-2"><div className="grid gap-2"><Label>يبدأ من</Label><Input type="datetime-local" value={editor.starts_at} onChange={event => setEditor(value => ({ ...value, starts_at: event.target.value }))} /></div><div className="grid gap-2"><Label>ينتهي في</Label><Input type="datetime-local" value={editor.ends_at} onChange={event => setEditor(value => ({ ...value, ends_at: event.target.value }))} /></div></div></div>
            <div className="grid gap-3"><div><Label>الفروع</Label><p className="text-xs text-muted-foreground">بدون اختيار = يظهر في كل الفروع.</p></div><div className="grid max-h-44 gap-2 overflow-y-auto rounded-2xl border p-3 sm:grid-cols-2">{state.branches.map((branch: HomeBuilderBranch) => { const checked = editor.branch_ids.includes(branch.id); return <label key={branch.id} className="flex cursor-pointer items-center gap-2 rounded-xl p-2 hover:bg-muted"><Checkbox checked={checked} onCheckedChange={() => setEditor(value => ({ ...value, branch_ids: checked ? value.branch_ids.filter(id => id !== branch.id) : [...value.branch_ids, branch.id] }))} /><span className="text-sm">{branch.name}</span></label>; })}</div></div>
          ...
          <DialogFooter className="gap-2"><Button variant="outline" onClick={() => setEditorOpen(false)} disabled={saving}>إلغاء</Button><Button onClick={() => void saveEditor()} disabled={saving} className="bg-[#005931] hover:bg-[#004525]">{saving && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}حفظ</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-lg" dir="rtl">
          <DialogHeader><DialogTitle>معاينة مسودة الصفحة</DialogTitle><DialogDescription>معاينة ترتيب المحتوى. في A/B سيشاهد كل عميل Variant واحد فقط بعد النشر.</DialogDescription></DialogHeader>
          <div className="mx-auto max-h-[70vh] w-full max-w-[390px] overflow-y-auto rounded-[32px] border-[6px] border-slate-900 bg-[#f6f8f7] p-3 shadow-2xl">
            ...
            {sections.filter(section => section.is_active).map(section => { const meta = typeMeta[section.section_type]; const Icon = meta.icon; return <div key={section.id} className={`mb-3 rounded-2xl border bg-white p-3 shadow-sm ${section.section_type === 'hero_banners' ? 'h-28' : ''}`}><div className="mb-2 flex items-center gap-2"><Icon className="h-4 w-4 text-[#005931]" /><strong className="text-sm">{section.title || meta.defaultTitle}</strong>{section.experiment_key && <Badge className="bg-indigo-100 text-indigo-800">{section.experiment_variant}</Badge>}</div><div className="h-12 rounded-xl bg-gradient-to-l from-emerald-50 to-slate-50" /><p className="mt-2 truncate text-[10px] text-muted-foreground">{audienceLabel(section)} · {branchLabel(section)}</p></div>; })}
          </div>
        </DialogContent>
      </Dialog>

      <AlertDialog open={publishOpen} onOpenChange={setPublishOpen}><AlertDialogContent dir="rtl"><AlertDialogHeader><AlertDialogTitle>نشر الصفحة الرئيسية؟</AlertDialogTitle><AlertDialogDescription>سيتم استبدال النسخة المنشورة بالمسودة v{state.draft.version} ثم إنشاء مسودة جديدة. الاستهداف وA/B والجدولة سيبدأون فورًا حسب الإعدادات.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={saving}>إلغاء</AlertDialogCancel><AlertDialogAction disabled={saving} onClick={event => { event.preventDefault(); void publish(); }} className="bg-[#005931] hover:bg-[#004525]">{saving && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}نشر الآن</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
      <AlertDialog open={Boolean(deleteTarget)} onOpenChange={open => !open && setDeleteTarget(null)}><AlertDialogContent dir="rtl"><AlertDialogHeader><AlertDialogTitle>حذف الجزء من المسودة؟</AlertDialogTitle><AlertDialogDescription>الحذف يؤثر على المسودة فقط ولن يغيّر النسخة المنشورة حتى تعمل نشر.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={saving}>إلغاء</AlertDialogCancel><AlertDialogAction disabled={saving} onClick={event => { event.preventDefault(); void confirmDelete(); }} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">{saving && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}حذف</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
    </MainLayout>
  );
};
