...
const formatQty = (value: number | null | undefined) => Number(value || 0).toLocaleString("ar-EG", { maximumFractionDigits: 3 });
...
export default function OperationsTasksPage() {
  ...
  const openInventory = async (task: OperationsTask) => {
    ...
    try {
      ...
      if (task.status !== "completed") {
        try { await startOperationsTask(task.id); } catch { /* idempotent/open transition can already be done */ }
      }
  ...
  const closeInventoryDialog = () => {
    setInventoryTask(null); setInventoryDetail(null); setActualCount(""); setCountNote(""); setBarcodeCheck(""); setAdjustmentNote("");
  };

  const submitInventory = async () => {
    if (!inventoryTask || !inventoryDetail || busyId || isInventoryAdjustmentReviewTask(inventoryTask)) return;
  ...
  const approveAdjustment = async () => {
    ...
    if (adjustmentNote.trim().length < 3) return toast.error("اكتب ملاحظة توضح سبب اعتماد التسوية.");
    if (Math.abs(Number(inventoryDetail.movement_ledger_gap || 0)) > 0.001) return toast.error("يوجد فرق في Movement Ledger. حدّث المهمة قبل الاعتماد.");
    setBusyId(inventoryTask.id);
    try {
  ...
  const rejectAdjustment = async () => {
    ...
    if (adjustmentNote.trim().length < 3) return toast.error("اكتب ملاحظة توضح سبب رفض التسوية.");
    setBusyId(inventoryTask.id);
    try {
  ...
  const TaskList = ({ rows, empty }: { rows: OperationsTask[]; empty: string }) => <div className="space-y-3">{rows.length ? rows.map(task => <TaskCard key={task.id} task={task} />) : <div className="rounded-3xl border border-dashed bg-white p-10 text-center text-sm text-muted-foreground">{empty}</div>}</div>;
  ...
  return (
    <MainLayout>
      <div dir="rtl" className="mx-auto max-w-[1500px] space-y-5 py-5">
        ...
        {query.isLoading ? <div className="flex min-h-[360px] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#005931]" /></div> : query.isError ? <div className="rounded-3xl border border-red-200 bg-red-50 p-6 text-red-800">{query.error instanceof Error ? query.error.message : "تعذر تحميل المهام."}</div> : (
          <Tabs defaultValue="mine" dir="rtl"><TabsList className="h-auto flex-wrap justify-start gap-1 rounded-2xl bg-white p-1.5 shadow-sm"><TabsTrigger value="mine">مهامي ({mine.length})</TabsTrigger><TabsTrigger value="available">متاحة ({available.length})</TabsTrigger><TabsTrigger value="active">نشطة ({active.length})</TabsTrigger><TabsTrigger value="overdue">متأخرة ({overdue.length})</TabsTrigger><TabsTrigger value="completed">مكتملة ({completed.length})</TabsTrigger></TabsList><TabsContent value="mine" className="mt-4"><TaskList rows={mine} empty="لا توجد مهام مسندة لك الآن." /></TabsContent><TabsContent value="available" className="mt-4"><TaskList rows={available} empty="لا توجد مهام متاحة للاستلام." /></TabsContent><TabsContent value="active" className="mt-4"><TaskList rows={active} empty="لا توجد مهام نشطة." /></TabsContent><TabsContent value="overdue" className="mt-4"><TaskList rows={overdue} empty="لا توجد مهام متأخرة." /></TabsContent><TabsContent value="completed" className="mt-4"><TaskList rows={completed} empty="لا توجد مهام مكتملة ضمن القائمة الحالية." /></TabsContent></Tabs>
        )}
      </div>
        ...
        <DialogContent dir="rtl" className="max-w-2xl">
          ...
          {!inventoryDetail ? <div className="flex min-h-40 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin" /></div> : <div className="space-y-4">
            ...
            {inventoryDetail.blind_count ? <>
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><strong>مراجعة قبل التسوية:</strong> النظام أعاد إسقاط كل الحركات التي حدثت بعد إعادة العد. لا يتم تعديل المخزون إلا بعد ضغط «اعتماد التسوية».</div>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-muted-foreground">رصيد النظام وقت إعادة العد</div><div className="mt-1 text-xl font-black">{formatQty(inventoryDetail.system_expected_at_recount)}</div></div>
                <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-muted-foreground">العد الأول</div><div className="mt-1 text-xl font-black">{formatQty(inventoryDetail.first_count)}</div></div>
                <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-muted-foreground">إعادة العد</div><div className="mt-1 text-xl font-black">{formatQty(inventoryDetail.recount)}</div></div>
                <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-muted-foreground">حركات بعد إعادة العد</div><div className="mt-1 text-xl font-black">{formatQty(inventoryDetail.post_recount_movement)}</div></div>
                <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-muted-foreground">الرصيد الحالي بالنظام</div><div className="mt-1 text-xl font-black">{formatQty(inventoryDetail.current_system_quantity)}</div></div>
                <div className="rounded-xl bg-cyan-50 p-3"><div className="text-xs text-muted-foreground">الرصيد الفعلي المتوقع بعد الحركات</div><div className="mt-1 text-xl font-black text-cyan-900">{formatQty(inventoryDetail.projected_physical_quantity)}</div></div>
              </div>
              <div className={`rounded-2xl border p-4 ${Math.abs(Number(inventoryDetail.current_adjustment_delta || 0)) > 0.001 ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-sm text-muted-foreground">التسوية المقترحة الآن</div><div className="mt-1 text-2xl font-black">{Number(inventoryDetail.current_adjustment_delta || 0) > 0 ? "+" : ""}{formatQty(inventoryDetail.current_adjustment_delta)} {inventoryDetail.unit_of_measure || "وحدة"}</div></div><div className="text-left"><div className="text-xs text-muted-foreground">قيمة فرق إعادة العد</div><div className="mt-1 font-black">{formatMoney(inventoryDetail.variance_value)}</div></div></div></div>
              {Math.abs(Number(inventoryDetail.movement_ledger_gap || 0)) > 0.001 && <div className="rounded-2xl border border-red-300 bg-red-50 p-4 text-sm font-bold text-red-800">Movement Ledger غير مطابق بفارق {formatQty(inventoryDetail.movement_ledger_gap)}. الاعتماد متوقف لحماية المخزون.</div>}
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2"><Label>سبب التسوية عند الاعتماد</Label><Select value={adjustmentReason} onValueChange={value => setAdjustmentReason(value as InventoryAdjustmentReason)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(adjustmentReasonLabels).map(([value,label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
                <div className="space-y-2"><Label>سبب عدم اعتماد التسوية</Label><Select value={rejectionReason} onValueChange={value => setRejectionReason(value as InventoryAdjustmentRejectionReason)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(rejectionReasonLabels).map(([value,label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
              </div>
              <div className="space-y-2"><Label htmlFor="adjustment-note">ملاحظة قرار المراجعة</Label><Textarea id="adjustment-note" value={adjustmentNote} onChange={e => setAdjustmentNote(e.target.value)} placeholder="اشرح سبب الاعتماد أو الرفض وما تم التحقق منه..." /></div>
            </>}
          </div>}
          <DialogFooter className="gap-2 sm:justify-start">
            ...
            {inventoryDetail && !inventoryDetail.blind_count && inventoryTask && inventoryTask.status !== "completed" && <><Button variant="destructive" onClick={rejectAdjustment} disabled={Boolean(busyId)}>رفض التسوية</Button><Button onClick={approveAdjustment} disabled={Boolean(busyId) || Math.abs(Number(inventoryDetail.movement_ledger_gap || 0)) > 0.001}>{busyId ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}اعتماد التسوية</Button></>}
          </DialogFooter>
        </DialogContent>
      </Dialog>
