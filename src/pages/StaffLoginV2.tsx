...
import { isPosNative, posCredentials } from "@/native/posNative";
import { fetchPosQuickStaff, getAnyLocalPosDevice, type PosQuickStaff } from "@/services/supabase/posDeviceService";
import { authenticatePartnerUser, resolvePortalAccess } from "@/services/supabase/portalAccessService";
...
export default function StaffLoginV2() {
  ...
  useEffect(() => {
    if (isLoading || isAuthenticated || branchSelectionRequired || isPosNative()) return;
    let active = true;
    void resolvePortalAccess().then(access => {
      if (active && access.kind === "partner") navigate("/partner", { replace: true });
      if (active && access.kind === "conflict") setLoginError("الحساب مرتبط بالنظامين. تواصل مع الإدارة لتحديد صلاحية واحدة.");
    }).catch(() => {
      // Keep the login form available if the session check cannot complete.
    });
    return () => { active = false; };
  }, [isLoading, isAuthenticated, branchSelectionRequired, navigate]);
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (username.includes("@")) {
        if (isPosNative()) throw new Error("دخول الشركاء متاح من نسخة الويب فقط.");
        await authenticatePartnerUser(username, password);
        navigate("/partner", { replace: true });
        return;
      }
      await login(username.trim(), password);
      if (isPosNative()) void posCredentials.savePassword({ username: username.trim(), password }).catch(error => console.warn("Google Password Manager unavailable", error));
    }
  ...
  const handleSavedPassword = async () => {
    ...
    try {
      setSubmitting(true);
      const saved = await posCredentials.getPassword();
      setUsername(saved.username);
      await login(saved.username, saved.password);
    } catch (error) {
      setLoginError((error as Error)?.message || "تعذر فتح كلمة المرور المحفوظة");
    } finally { setSubmitting(false); }
  };
  ...
  const shell = (content: React.ReactNode, hint?: React.ReactNode) => (
    <div dir="rtl" className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[minmax(0,1fr)_420px]">
      <section className="relative hidden overflow-hidden bg-[#005931] p-10 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative z-10"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/12 ring-1 ring-white/20"><Store className="h-7 w-7" /></div><h1 className="mt-6 text-4xl font-black">ماركت المعداوي</h1><p className="mt-3 max-w-xl text-lg leading-8 text-emerald-50">دخول واحد للموظفين والشركاء. كل حساب يفتح مساحة العمل المخصصة له فقط.</p></div>
      ...
      <main className="flex min-h-screen items-center justify-center p-4 sm:p-8">
        <div className="w-full max-w-md">
          ...
          {hint}
          {content}
          <p className="mt-5 text-center text-[11px] leading-5 text-slate-400">الموظف يستخدم اسم المستخدم، والشريك يستخدم البريد الإلكتروني. لا تشارك كلمة المرور أو PIN مع أي شخص.</p>
        </div>
      </main>
    </div>
  );
  ...
  if (localDevice && !showFullLogin) {
    ...
    return shell(<Card className="rounded-3xl border-0 shadow-xl shadow-slate-900/5"><CardHeader><div className="mb-2 flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-50 text-[#005931]"><LogIn className="h-5 w-5" /></div><CardTitle className="text-2xl font-black">تسجيل الدخول</CardTitle><CardDescription>الموظف يدخل باسم المستخدم، والشريك بالبريد الإلكتروني. هنفتح المساحة المسموحة لحسابك تلقائيًا.</CardDescription></CardHeader><CardContent>{loginError && <Alert variant="destructive" className="mb-4"><AlertTriangle className="h-4 w-4" /><AlertDescription>{loginError}</AlertDescription></Alert>}<form onSubmit={handleLogin} className="space-y-5"><div className="space-y-2"><Label htmlFor="username">اسم المستخدم أو البريد الإلكتروني</Label><Input id="username" autoComplete="username" value={username} onChange={event => setUsername(event.target.value)} className="h-12 rounded-xl" placeholder="اسم المستخدم أو البريد الإلكتروني" disabled={busy} /></div><div className="space-y-2"><Label htmlFor="password">كلمة المرور</Label><div className="relative"><Input id="password" type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} className="h-12 rounded-xl pl-11" placeholder="كلمة المرور" disabled={busy} /><button type="button" className="absolute inset-y-0 left-0 flex w-11 items-center justify-center text-slate-400 hover:text-slate-700" onClick={() => setShowPassword(value => !value)} aria-label={showPassword ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></div><Button type="submit" className="h-12 w-full rounded-xl bg-[#005931] hover:bg-[#004526]" disabled={busy}>{busy ? <RefreshCw className="ml-2 h-4 w-4 animate-spin" /> : <LogIn className="ml-2 h-4 w-4" />}تسجيل الدخول</Button>{isPosNative() && <Button type="button" variant="outline" className="h-12 w-full rounded-xl" disabled={busy} onClick={() => void handleSavedPassword()}><KeyRound className="ml-2 h-4 w-4" />استخدام كلمة مرور محفوظة من جوجل</Button>}</form>{localDevice && <Button variant="ghost" className="mt-3 w-full" onClick={() => { setShowFullLogin(false); setLoginError(null); }}>الرجوع للدخول السريع للكاشير</Button>}</CardContent></Card>);
}
