...
export default function StaffLoginV2() {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try { setSubmitting(true); await login(username.trim(), password); if (isPosNative()) void posCredentials.savePassword({ username: username.trim(), password }).catch(error => console.warn("Google Password Manager unavailable", error)); }
