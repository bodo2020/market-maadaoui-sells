...
function voucherContextKey(branchId: string, checkoutId: string) {
  return `pos-loyalty-voucher:${branchId}:${checkoutId}`;
}
...
function readStoredLoyaltyCustomer(branchId: string, checkoutId: string) {
  return readStoredJson<StoredLoyaltyCustomer>(loyaltyContextKey(branchId, checkoutId));
}

function readStoredLoyaltyVoucher(branchId: string, checkoutId: string) {
  return readStoredJson<StoredLoyaltyVoucher>(voucherContextKey(branchId, checkoutId));
}
...
function friendlySaleError(message?: string) {
  ...
  if (value.includes("POS_SHIFT_REQUIRED") || value.includes("SHIFT_NOT_OPEN") || value.includes("POS_SHIFT_CASH_ACCOUNT_MISSING")) return "لا توجد وردية POS مفتوحة لهذا الموظف على الجهاز الحالي.";
  ...
  if (value.includes("INVALID_PAYMENT_SPLIT")) return "تقسيم الدفع غير صحيح. النقدي + البطاقة + الفاوچر لازم يساووا إجمالي الفاتورة.";
  ...
  if (value.includes("CUSTOMER_REQUIRED_FOR_VOUCHER")) return "اربط العميل بالفاتورة قبل استخدام الفاوچر.";
  if (value.includes("VOUCHER_NOT_FOUND")) return "الفاوچر غير موجود. امسح الباركود مرة أخرى.";
  if (value.includes("VOUCHER_CUSTOMER_MISMATCH")) return "الفاوچر لا يخص العميل المرتبط بالفاتورة.";
  if (value.includes("VOUCHER_UNAVAILABLE")) return "الفاوچر مستخدم بالكامل أو غير متاح.";
  if (value.includes("INVALID_VOUCHER_AMOUNT") || value.includes("VOUCHER_AMOUNT_TOO_HIGH")) return "قيمة الفاوچر غير صالحة لهذه الفاتورة.";
  if (value.includes("VOUCHER_BALANCE_CHANGED")) return "رصيد الفاوچر اتغير. امسحه مرة تانية قبل التأكيد.";
...
export async function submitPosSale(
  ...
  if (typeof navigator !== "undefined" && !navigator.onLine) throw new Error("الإنترنت مقطوع حاليًا. السلة محفوظة؛ رجّع الاتصال وحاول بنفس السلة.");
  ...
  const loyaltyVoucher = readStoredLoyaltyVoucher(branchId, checkoutId);
  ...
  if (loyaltyVoucher && !customerId) throw new Error("اربط العميل بالفاتورة قبل استخدام الفاوچر.");
  if (loyaltyVoucher?.customer_id && customerId && loyaltyVoucher.customer_id !== customerId) throw new Error("الفاوچر لا يخص العميل المرتبط بالفاتورة.");

  const voucherAmount = loyaltyVoucher
    ? Math.max(0, Math.min(Number(loyaltyVoucher.remaining_value_egp || 0), Number(sale.total || 0)))
    : 0;
  ...
  const payload = {
    ...
    voucher_code: loyaltyVoucher?.voucher_code || loyaltyVoucher?.barcode_token || null,
  ...
  try {
    const raw = localStorage.getItem(key);
    pending = raw ? (JSON.parse(raw) as PendingSale) : { requestId: crypto.randomUUID(), fingerprint, payload: { ...payload, invoice_number: sale.invoice_number } };
  } catch {
    pending = { requestId: crypto.randomUUID(), fingerprint, payload: { ...payload, invoice_number: sale.invoice_number } };
  }

  if (pending.fingerprint !== fingerprint) throw new Error("فيه محاولة حفظ سابقة لنفس السلة. راجع الفاتورة السابقة قبل تغيير بيانات الدفع أو الفاوچر.");

  try { localStorage.setItem(key, JSON.stringify(pending)); } catch { /* server request id remains authoritative */ }
  ...
  if (result.error && !isDeterministicError(result.error.code, result.error.message)) {
    ...
    void logPosOperationalEvent(branchId, "checkout_retry", "warning", result.error.message || result.error.code || "NETWORK_UNCERTAIN", {
      request_id: pending.requestId, checkout_id: checkoutId, item_count: sale.items.length, total: Number(sale.total || 0),
      payment_method: sale.payment_method, customer_id: customerId, voucher_id: loyaltyVoucher?.id || null, voucher_amount: voucherAmount,
    });
  ...
  if (result.error) {
    ...
    const severity = result.error.message?.includes("PRICE_CHANGED") || result.error.message?.includes("INSUFFICIENT_STOCK") ? "warning" : "error";
    void logPosOperationalEvent(branchId, "checkout_error", severity, result.error.message || result.error.code || "CHECKOUT_ERROR", {
      request_id: pending.requestId, checkout_id: checkoutId, item_count: sale.items.length, total: Number(sale.total || 0),
      payment_method: sale.payment_method, customer_id: customerId, voucher_id: loyaltyVoucher?.id || null,
      voucher_amount: voucherAmount, rpc_code: result.error.code || null, duration_ms: elapsedMs, retried,
    });
  ...
  if (!result.data) throw new Error("لم يصل تأكيد البيع. السلة محفوظة؛ أعد المحاولة نفسها ولن تُسجّل الفاتورة مرتين.");

  const confirmedSale = result.data as Sale;

  if (elapsedMs >= 1500) {
    void logPosOperationalEvent(branchId, "checkout_slow", "warning", "SLOW_CHECKOUT", {
      request_id: pending.requestId, checkout_id: checkoutId, duration_ms: elapsedMs, retried, item_count: sale.items.length,
      total: Number(sale.total || 0), payment_method: sale.payment_method, customer_id: customerId,
      voucher_id: loyaltyVoucher?.id || null, voucher_amount: voucherAmount,
    });
  }
  ...
  try {
    ...
    localStorage.removeItem(voucherContextKey(branchId, checkoutId));
  } catch {
    // Sale is already committed; local cleanup failure must not report a sale failure.
  }

  invalidatePOSCatalogCache(branchId);
  invalidatePosPreflightCache(branchId);
  if (typeof window !== "undefined") window.dispatchEvent(new CustomEvent<Sale>("pos:sale-completed", { detail: confirmedSale }));
