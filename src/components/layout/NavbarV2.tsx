import { Bell, ChevronDown, Check, Menu, Settings, Store, UserRound, LogOut, BriefcaseBusiness, KeyRound } from "lucide-react";
...
export default function NavbarV2({ onMenuClick, isMobile = false }: { onMenuClick?: () => void; isMobile?: boolean }) {
  ...
  return (
    <header dir="rtl" className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
      <div className="flex min-h-[68px] items-center gap-2 px-3 md:px-5">
        ...
        <div className="flex items-center gap-1.5 md:gap-2">
          {branchOptions.length > 1 ? <DropdownMenu>
            <DropdownMenuTrigger asChild><Button variant="outline" className="h-11 max-w-[180px] gap-2 rounded-2xl px-3"><Store className="h-4 w-4 shrink-0" /><span className="truncate hidden sm:inline">{currentBranchName || "اختر الفرع"}</span><ChevronDown className="h-4 w-4 shrink-0" /></Button></DropdownMenuTrigger>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-64" dir="rtl">
              <DropdownMenuLabel><div className="font-black">{user?.name}</div><div className="mt-1 text-[11px] font-normal text-muted-foreground">{roleLabel} · {currentBranchName || "بدون فرع"}</div></DropdownMenuLabel>
              ...
              <DropdownMenuItem onClick={() => navigate("/settings")}><Settings className="ml-2 h-4 w-4" />الإعدادات</DropdownMenuItem>
              <DropdownMenuSeparator />
      ...
      {actionRequired > 0 && <div className="flex items-center justify-between border-t border-amber-100 bg-amber-50 px-3 py-1.5 text-xs text-amber-900 lg:hidden"><span>عندك {actionRequired.toLocaleString("ar-EG")} إجراء يحتاج متابعة</span><button className="font-black underline" onClick={() => navigate("/tasks")}>فتح المهام</button></div>}
    </header>
  );
}
