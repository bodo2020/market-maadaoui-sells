...
import { BarChart3, Boxes, Clock, PackageSearch, ReceiptText, RotateCcw, TrendingUp, WalletCards } from "lucide-react";
...
export default function ReportsSectionNav() {
  return (
    <nav
      ...
      className="flex w-full gap-2 overflow-x-auto rounded-2xl border bg-background/80 p-1.5 shadow-sm"
    >
      {items.map((item) => {
        ...
        return (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "flex min-w-max items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-colors",
                isActive
