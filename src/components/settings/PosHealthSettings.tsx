...
export default function PosHealthSettings() {
  ...
  const load = async (quiet = false) => {
    ...
    try {
      const rows = await listBranchPosOperationalEvents(currentBranchId, filter === "all" ? null : filter, 100);
      setEvents(rows);
      setError(null);
    } catch (e: any) {
    ...
    <div dir="rtl" className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{currentBranchName || "الفرع الحالي"} · سجل تقني مختصر للأحداث المهمة على أجهزة الكاشير.</p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card><CardContent className="p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><MonitorSmartphone className="h-4 w-4" /> أجهزة ظهر عليها حدث</div><div className="mt-2 text-2xl font-black">{affectedDevices}</div></CardContent></Card>
      </div>
