...
import { AiProvider, AiRuntimeSettings, fetchAiRuntimeSettings, saveAiRuntimeSettings } from "@/services/supabase/aiGatewayService";
...
const defaults: AiRuntimeSettings = {
  ...
  tertiary_model: "openrouter/auto",
  timeout_ms: 20000,
  ...
  return (
    <div className="space-y-4" dir="rtl">
      <Alert className="border-amber-200 bg-amber-50 text-amber-950">
        ...
        <AlertDescription>مفاتيح Gemini وGroq وOpenRouter محفوظة كـSupabase Secrets فقط ولا تظهر في المتصفح أو قاعدة البيانات.</AlertDescription>
      </Alert>

      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Bot className="h-5 w-5 text-[#005931]" />Elmadawy AI Gateway</CardTitle>
          <CardDescription>تحكم مركزي في سلسلة المزودين وحدود الرد. الترتيب الافتراضي: Gemini ثم Groq ثم OpenRouter.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-5 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Select value={settings.primary_provider} onValueChange={(value: AiProvider) => setSettings((s) => ({ ...s, primary_provider: value }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{providerOptions.map((provider) => <SelectItem key={provider.value} value={provider.value}>{provider.label}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2">
            <Label>المزود الاحتياطي الأول</Label>
            <Select value={settings.fallback_provider || "none"} onValueChange={(value) => setSettings((s) => ({ ...s, fallback_provider: value === "none" ? null : value as AiProvider }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{providerOptions.map((provider) => <SelectItem key={provider.value} value={provider.value}>{provider.label}</SelectItem>)}<SelectItem value="none">بدون احتياطي</SelectItem></SelectContent>
            </Select>
          </div>
          <div className="space-y-2"><Label>موديل الاحتياطي الأول</Label><Input dir="ltr" disabled={!settings.fallback_provider} value={settings.fallback_model || ""} onChange={(event) => setSettings((s) => ({ ...s, fallback_model: event.target.value || null }))} /></div>

          <div className="space-y-2">
            <Label>المزود الاحتياطي الثاني</Label>
            <Select value={settings.tertiary_provider || "none"} onValueChange={(value) => setSettings((s) => ({ ...s, tertiary_provider: value === "none" ? null : value as AiProvider }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{providerOptions.map((provider) => <SelectItem key={provider.value} value={provider.value}>{provider.label}</SelectItem>)}<SelectItem value="none">بدون احتياطي</SelectItem></SelectContent>
            </Select>
          </div>
          <div className="space-y-2"><Label>موديل الاحتياطي الثاني</Label><Input dir="ltr" disabled={!settings.tertiary_provider} value={settings.tertiary_model || ""} onChange={(event) => setSettings((s) => ({ ...s, tertiary_model: event.target.value || null }))} /></div>
