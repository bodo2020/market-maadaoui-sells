...
import { AlertTriangle, Banknote, LockKeyhole, RefreshCw, ShieldAlert, WifiOff } from "lucide-react";
...
function PinKeypad({ onDigit, onClear, onBackspace, disabled }: { onDigit: (digit: string) => void; onClear: () => void; onBackspace: () => void; disabled: boolean; }) {
  return (
    <div className="grid grid-cols-3 gap-2" dir="ltr">
      {["1","2","3","4","5","6","7","8","9"].map(digit => <Button key={digit} type="button" variant="outline" className="h-14 text-lg" disabled={disabled} onClick={() => onDigit(digit)}>{digit}</Button>)}
...
export default function PosRuntimeGuard({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const activity = () => { if (!locked) armTimer(); };
  ...
  useEffect(() => { if (!locked) armTimer(); }, [autoLockMinutes, locked, armTimer]);
  ...
  const unlock = async () => {
    ...
    try {
      ...
      const status = await getPosRuntimeStatus(device);
      applyRuntimeSettings(status);
      if (!status.ready) { setRuntimeIssue("الوردية لم تعد مفتوحة. حدّث الصفحة قبل استكمال البيع."); return; }
      setRuntimeIssue(null); setPin(""); setLocked(false); armTimer();
    } catch (e: any) { setPin(""); setError(e?.message || "PIN غير صحيح"); }
    finally { setUnlocking(false); }
  };

  const cashWarningActive = online && !locked && !runtimeIssue && cashWarningThreshold != null && cashWarningThreshold > 0 && drawerBalance != null && drawerBalance >= cashWarningThreshold;
  ...
  return (
    <>
      ...
      {checkingRuntime && !locked && !runtimeIssue && <div className="pointer-events-none fixed left-3 top-32 z-[60] rounded-full bg-white/90 p-2 text-slate-400 shadow-sm"><RefreshCw className="h-3.5 w-3.5 animate-spin" /></div>}
      ...
      {locked && !runtimeIssue && <div dir="rtl" className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-md"><div className="w-full max-w-sm rounded-3xl bg-white p-5 shadow-2xl"><div className="text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-50 text-[#005931]"><LockKeyhole className="h-7 w-7" /></div><h2 className="mt-3 text-xl font-black">الكاشير مقفول</h2><p className="mt-1 text-sm text-slate-500">{user?.name || "الموظف الحالي"} · الوردية والسلات ما زالت مفتوحة</p></div>{!online && <div className="mt-4 flex items-start gap-2 rounded-xl bg-amber-50 p-3 text-xs text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /> لازم الاتصال يرجع قبل التحقق من PIN.</div>}{error && <div className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}<div className="my-5 flex justify-center gap-2" dir="ltr">{Array.from({ length: 6 }).map((_, index) => <span key={index} className={`h-3 w-3 rounded-full ${index < pin.length ? "bg-[#005931]" : "bg-slate-200"}`} />)}</div><PinKeypad disabled={unlocking} onDigit={addDigit} onClear={() => { setPin(""); setError(null); }} onBackspace={() => setPin(value => value.slice(0,-1))} /><Button className="mt-4 h-12 w-full bg-[#005931] hover:bg-[#004a29]" disabled={!online || unlocking || pin.length < 4} onClick={() => void unlock()}>{unlocking ? <RefreshCw className="h-4 w-4 animate-spin" /> : <LockKeyhole className="h-4 w-4" />} فتح بالـPIN</Button></div></div>}
    </>
  );
}
