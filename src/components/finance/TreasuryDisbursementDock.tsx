...
import { AlertTriangle, Banknote, CheckCircle2, Landmark, Loader2, RefreshCw, ShieldCheck, Vault, XCircle } from "lucide-react";
...
const payrollMonth = (month: number, year: number) => new Intl.DateTimeFormat("ar-EG", { month: "long", year: "numeric" }).format(new Date(year, month - 1, 1));
...
export default function TreasuryDisbursementDock() {
  ...
  const [payrollDetail, setPayrollDetail] = useState<PayrollTreasuryTaskDetail | null>(null);
  ...
  const tasks = useMemo(() => (query.data || []).filter(task => {
    ...
    if (task.source_kind === "hr_treasury_payout" || task.source_kind === "hr_treasury_payroll") return Boolean(task.is_mine);
  ...
  const isPayrollTask = selected?.source_kind === "hr_treasury_payroll";
  const isAdvanceTreasuryTask = selected?.source_kind === "hr_treasury_payout";
  ...
  const openTask = async (task: OperationsTask) => {
    ...
    try {
      ...
      } else if (task.source_kind === "hr_treasury_payroll") {
        try { await startOperationsTask(task.id); } catch { /* already started */ }
        const detail = await getPayrollTreasuryTask(task.id);
        setSelected(task);
  ...
  const confirm = async () => {
    if (!selected || (!isAdvanceTreasuryTask && !isPayrollTask)) return;
    if (note.trim().length < 3) return toast.error("اكتب ملاحظة تؤكد تنفيذ الصرف.");
    if (reference.trim().length < 2) return toast.error("أدخل مرجع الصرف قبل التأكيد.");
    setBusy(true);
    try {
      if (isPayrollTask) {
        const result = await confirmPayrollTreasuryPayout(selected.id, note.trim(), reference.trim());
        toast.success(`تم صرف المسير وخصم ${money(result.amount)} من ${result.source_account_name || "مصدر العهدة"}. الرصيد الآن ${money(result.source_balance_after)}.`);
      } else {
        const result = await confirmTreasuryPayout(selected.id, note.trim(), reference.trim());
  ...
  const reject = async () => {
    ...
    try {
      if (isPayrollTask) {
        await rejectPayrollTreasuryPayout(selected.id, note.trim());
        toast.success("تم رفض صرف المسير وإعادته للمالية بحالة مقفل، بدون خصم أي مبلغ.");
      } else {
  ...
  return <>
    <Sheet open={panelOpen} onOpenChange={setPanelOpen}>
      <SheetContent side="left" dir="rtl" className="w-full overflow-y-auto sm:max-w-lg">
        <SheetHeader className="text-right">
          ...
          <SheetDescription>السلف ومسيرات الرواتب: الخصم الفعلي لا يحدث إلا بعد تأكيد مسؤول العهدة.</SheetDescription>
        </SheetHeader>
        ...
        <div className="mt-3 space-y-2">
          {query.isLoading ? <div className="flex justify-center p-8"><Loader2 className="h-6 w-6 animate-spin" /></div> : tasks.length === 0 ? <div className="rounded-2xl border border-dashed bg-slate-50 p-8 text-center text-sm text-muted-foreground">لا توجد عمليات صرف تحتاج إجراء منك.</div> : tasks.map(task => {
            const payroll = task.source_kind === "hr_treasury_payroll";
            const treasury = payroll || task.source_kind === "hr_treasury_payout";
            const payrollAmount = payroll ? Number(task.metadata?.total_net || 0) : Number(task.amount || 0);
            return <div key={task.id} className={`rounded-2xl border bg-white p-4 ${payroll ? "border-blue-200" : ""}`}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="flex items-center gap-2 font-black">{payroll ? <Landmark className="h-4 w-4 text-blue-700" /> : treasury ? <Vault className="h-4 w-4 text-amber-700" /> : <Banknote className="h-4 w-4 text-emerald-700" />}{payroll ? "صرف مسير رواتب من عهدتك" : treasury ? "مهمة صرف من عهدتك" : task.title}</div>
                  <div className="mt-1 text-xs leading-5 text-muted-foreground">{payroll ? `صافي المسير ${money(payrollAmount)} · لن يتحول Paid قبل تأكيدك.` : treasury ? "المبلغ لا يخصم إلا بعد تأكيدك الفعلي للصرف." : "حدد الخزنة ليتم إرسال المهمة لمسؤولها."}</div>
                </div>
                {task.is_overdue && <Badge variant="destructive">متأخرة</Badge>}
              </div>
              <Button className="mt-3 w-full" size="sm" disabled={busy || (!treasury && !task.is_mine && !task.can_claim)} onClick={() => void openTask(task)}>{treasury ? <ShieldCheck className="ml-1 h-4 w-4" /> : <Banknote className="ml-1 h-4 w-4" />}{payroll ? "مراجعة وصرف المسير" : treasury ? "مراجعة الصرف" : "تحديد مصدر الصرف"}</Button>
            </div>;
          })}
        </div>
      </SheetContent>
    ...
    <Dialog open={Boolean(selected)} onOpenChange={value => { if (!value) close(); }}>
      <DialogContent dir="rtl" className="max-w-xl">
        <DialogHeader><DialogTitle>{isPayrollTask ? "تأكيد صرف مسير الرواتب" : isAdvanceTreasuryTask ? "تأكيد صرف من الخزنة" : "إسناد صرف سلفة إلى خزنة"}</DialogTitle></DialogHeader>
        ...
        {isAdvanceTreasuryTask && treasuryDetail && <div className="space-y-4">
        ...
        {isPayrollTask && payrollDetail && <div className="space-y-4">
          <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
            <div className="flex items-center gap-2 font-black text-blue-950"><Landmark className="h-5 w-5" />مسير {payrollMonth(payrollDetail.payroll.month, payrollDetail.payroll.year)}</div>
            <div className="mt-3 text-3xl font-black text-blue-950">{money(payrollDetail.payroll.total_net)}</div>
            <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><div>مصدر الصرف: <strong>{payrollDetail.source.account_name}</strong></div><div>الرصيد الحالي: <strong>{money(payrollDetail.source.balance)}</strong></div></div>
          </div>
          <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm leading-6 text-red-950"><strong>تأكيد الصرف إجراء مالي نهائي.</strong> عند التأكيد سيتم خصم صافي المسير مرة واحدة من العهدة، تحويل المسير إلى Paid، تثبيت سجلات الرواتب، وتطبيق أقساط السلف المدرجة. لو لم يتم التسليم/التحويل اختر الرفض؛ لن يُخصم شيء.</div>
          <div className="space-y-2"><Label>مرجع الصرف</Label><Input value={reference} onChange={e => setReference(e.target.value)} placeholder="رقم التحويل / سند الصرف" /></div>
          <div className="space-y-2"><Label>ملاحظة التنفيذ أو سبب الرفض</Label><Textarea rows={4} value={note} onChange={e => setNote(e.target.value)} placeholder="مثال: تم تحويل الرواتب طبقًا لكشف المسير ومراجعة الإجمالي..." /></div>
        </div>}
        ...
        <DialogFooter className="gap-2 sm:justify-start">
          ...
          {(isAdvanceTreasuryTask || isPayrollTask) && <><Button variant="destructive" disabled={busy || note.trim().length < 3} onClick={() => void reject()}>{busy ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <XCircle className="ml-2 h-4 w-4" />}رفض / تعذر الصرف</Button><Button disabled={busy || note.trim().length < 3 || reference.trim().length < 2} onClick={() => void confirm()}>{busy ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}{isPayrollTask ? "تأكيد صرف المسير" : "تم الصرف وتأكيد الخصم"}</Button></>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </>;
