...
export default function TreasuryDisbursementDock() {
  ...
  const close = () => {
    setSelected(null); setTreasuryDetail(null); setSources([]); setSourceId(""); setNote(""); setReference("");
  };
  ...
  const openTask = async (task: OperationsTask) => {
    ...
    try {
      if (task.source_kind === "hr_salary_advance_payout") {
        ...
        try { current = await startOperationsTask(current.id) as OperationsTask; } catch { /* may already be started */ }
        ...
        try { await startOperationsTask(task.id); } catch { /* task may be already in progress */ }
  ...
  return <>
    <div dir="rtl" className="fixed bottom-5 right-5 z-40 w-[min(92vw,450px)]">
      <Card className="overflow-hidden border-amber-200 shadow-2xl">
        <CardContent className="p-0">
          <button type="button" className="flex w-full items-center justify-between gap-3 bg-white p-4 text-right" onClick={() => setExpanded(value => !value)}>
            <div className="flex items-center gap-3"><div className="rounded-xl bg-amber-50 p-2 text-amber-800"><Vault className="h-5 w-5" /></div><div><div className="font-black">صرف من الخزن</div><div className="text-xs text-muted-foreground">إسناد المالية + تأكيد أمين الخزنة</div></div></div>
            <Badge className="bg-amber-700">{tasks.length.toLocaleString("ar-EG")}</Badge>
          </button>
          {expanded && <div className="max-h-[52vh] space-y-2 overflow-auto border-t bg-slate-50 p-3">
            ...
            {query.isLoading ? <div className="flex justify-center p-5"><Loader2 className="h-5 w-5 animate-spin" /></div> : tasks.length === 0 ? <div className="rounded-xl border border-dashed bg-white p-5 text-center text-sm text-muted-foreground">لا توجد عمليات صرف تحتاج إجراء منك.</div> : tasks.map(task => {
              const treasury = task.source_kind === "hr_treasury_payout";
              return <div key={task.id} className="rounded-xl border bg-white p-3"><div className="flex items-start justify-between gap-2"><div><div className="flex items-center gap-2 font-black">{treasury ? <Vault className="h-4 w-4 text-amber-700" /> : <Banknote className="h-4 w-4 text-emerald-700" />}{treasury ? "مهمة صرف من عهدتك" : task.title}</div><div className="mt-1 text-xs text-muted-foreground">{treasury ? "المبلغ لا يخصم إلا بعد تأكيدك الفعلي للصرف." : "حدد الخزنة ليتم إرسال المهمة لمسؤولها."}</div></div>{task.is_overdue && <Badge variant="destructive">متأخرة</Badge>}</div><Button className="mt-3 w-full" size="sm" disabled={busy || (!treasury && !task.is_mine && !task.can_claim)} onClick={() => openTask(task)}>{treasury ? <ShieldCheck className="ml-1 h-4 w-4" /> : <Banknote className="ml-1 h-4 w-4" />}{treasury ? "مراجعة الصرف" : "تحديد مصدر الصرف"}</Button></div>;
            })}
          </div>}
        </CardContent>
      </Card>
    ...
    <Dialog open={Boolean(selected)} onOpenChange={value => { if (!value) close(); }}>
      <DialogContent dir="rtl" className="max-w-xl">
        ...
        {selected?.source_kind === "hr_treasury_payout" && treasuryDetail && <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-red-100 bg-red-50 p-4 text-sm leading-6 text-red-900"><strong>تأكيد الصرف = خصم فعلي.</strong> لا تضغط تأكيد إلا بعد تسليم المبلغ فعليًا. النظام يعيد فحص أن الخزنة ما زالت في عهدتك وأن الرصيد يكفي.</div>
        ...
        <DialogFooter className="gap-2 sm:justify-start">
          ...
          {selected?.source_kind === "hr_salary_advance_payout" && <Button disabled={busy || !sourceId} onClick={delegate}>{busy ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <ShieldCheck className="ml-2 h-4 w-4" />}إرسال لمسؤول الخزنة</Button>}
          {selected?.source_kind === "hr_treasury_payout" && <><Button variant="destructive" disabled={busy} onClick={reject}>{busy ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <XCircle className="ml-2 h-4 w-4" />}رفض / تعذر الصرف</Button><Button disabled={busy} onClick={confirm}>{busy ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}تم الصرف وتأكيد الخصم</Button></>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </>;
