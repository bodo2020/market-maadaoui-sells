...
import { AlertTriangle, CheckCircle2, ClipboardCheck, Clock3, Gauge, ListChecks, PackageCheck, Timer } from "lucide-react";
...
export default function EmployeePerformancePanel({ employeeId, branchId }: { employeeId: string; branchId: string | null }) {
  ...
  const query = useQuery({
    queryKey: ["hr-employee-performance-v1", employeeId, branchId, days],
  ...
  if (query.isLoading) return <Card><CardContent className="p-8 text-center text-muted-foreground">جاري تجميع مؤشرات الأداء من الحضور والمهام...</CardContent></Card>;
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="gap-4 md:flex-row md:items-center md:justify-between">
          <div><CardTitle className="flex items-center gap-2"><ClipboardCheck className="h-5 w-5 text-[#005931]" />الأداء والالتزام</CardTitle><CardDescription>مؤشرات تشغيلية قابلة للتفسير، بدون درجة Productivity مصطنعة.</CardDescription></div>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="rounded-2xl border border-blue-200 bg-blue-50/60 p-4 text-sm leading-6 text-blue-950"><div className="flex items-center gap-2 font-black"><CheckCircle2 className="h-4 w-4" />تفسير المؤشرات</div><p className="mt-1">دقة الجرد = نسبة مهام العد التي طابقت الرصيد المتوقع. لا نستخدمها وحدها للحكم على الموظف، لأن اكتشاف فرق حقيقي في المخزون قد يكون أداءً جيدًا وليس خطأ.</p><p className="mt-1">الغياب الكامل غير محسوب في V1 حتى نربط Scheduler بسجل يومي يولَّد حتى عند عدم تسجيل حضور.</p></div>
        </CardContent>
      </Card>
    </div>
  );
